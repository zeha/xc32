...
python ./compilerpage_automate/compauto.py $NOTIFY_TEST $SET_LATEST xc32 internal ${VERSION} xc32-v* xc32*.zip README.html
...
if [[ "x${bamboo_RUN_LTAUTOLIGHTS}" == "xtrue" || "x${bamboo_RUN_LTAUTOLIGHTS}" == "xTRUE" ]]; then
  mv .LanguageToolSuite ./build-scripts/autolights/xc32.LanguageToolSuite
  cd ./build-scripts/autolights
