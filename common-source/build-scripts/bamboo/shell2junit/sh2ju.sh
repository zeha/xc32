...
#
# Make suite=name below to get individual xml for each test
#
...
juLog() {
  ...
  # Let test suite name = test name
  suite=`echo $name | sed -e 's/[ \t]//g'`
  ...
  ## failure tag
  errmsg=`echo $H | sed -e 's~&~\&amp;~g' -e 's~<~\&lt;~g'  -e  's~>~\&gt;~g' -e 's~\"~\&quot;~g'`
  [ $err = 0 ] && failure="" || failure="
      <failure type=\"ScriptError\" message=\"$errmsg\"></failure>
  "
  ## testcase tag
  content="$content
