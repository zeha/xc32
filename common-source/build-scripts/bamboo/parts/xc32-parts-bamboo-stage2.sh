...
if [ ! -d install-Linux-nolm ]; then
  echo "No compiler artifact install-Linux-nolm found, check artifact download in stage 2"
  echo "PWD is:"
  pwd
  exit 1
fi

export PATH=$(pwd)/install-Linux-nolm/bin:$(pwd)/install-Linux-nolm/bin/bin:$PATH
