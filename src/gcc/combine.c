...
try_combine (rtx i3, rtx i2, rtx i1, int *new_direct_jump_p)
{
 ...
 validate_replacement:
  ...
  /* Check for case where newpat is 
     parallel (set regA = expr)
              (set regB = expr)
     If expr contains regB as one of its sub-expressions, we want to swap the 
     sequence such that regB = expr happens first. The allows the deadcode / 
     redundent elimination of regA by CSE since regA is just a copy of regB.
  ...
  if (insn_code_number < 0 && GET_CODE (newpat) == PARALLEL
      ...
      && asm_noperands (newpat) < 0)
    {
	  ...
	  !rtx_equal_p (SET_DEST (set0), SET_DEST (set1)))
	if (GET_CODE (SET_DEST (set0)) == REG 
	    && GET_CODE (SET_DEST (set1)) == REG)
	  /* We are trying to re-use registers before introducing a new set,
	     hence we check for occurrences of regB in expr.
	     But if regA also appears in expr, we will not gain anything, and
	     there's a potential problem with inline if assigments. */
	  if (reg_overlap_mentioned_p (SET_DEST (set0), SET_SRC (set0))
	      && !reg_overlap_mentioned_p (SET_DEST (set1), SET_SRC (set1)))
	    {
	      rtx set0_dest = SET_DEST (set0);
	      rtx set1_dest = SET_DEST (set1);
    	      SET_SRC (set1) = SET_DEST (set0);

	      if (next_real_insn (i2) == i3 
		  || !use_crosses_set_p (set0, INSN_CUID (i2)))
		{
		  rtx newi3pat = set1;
		  
		  i2_code_number = recog_for_combine (&set0, i2, &new_i2_notes);
		  
		  if (i2_code_number >= 0 
		      && (next_real_insn (i2) == i3
			  || ! reg_used_between_p (set0_dest, i2, i3)))
		  ...
		  if (insn_code_number >= 0)
		    {
		      ...
		      remove_note (i2, find_regno_note (i2, REG_DEAD, 
							REGNO (set0_dest)));
		      remove_note (i3, find_regno_note (i3, REG_DEAD, 
							REGNO (set1_dest)));

		      for (scan_insn = NEXT_INSN (i3); 
			   scan_insn && scan_insn 
			     != NEXT_INSN (BB_END (this_basic_block));
			   scan_insn = NEXT_INSN (scan_insn))
			if (INSN_P (scan_insn))
			  {
			    rtx cur_link;
			    for (cur_link = LOG_LINKS (scan_insn); 
				 cur_link; cur_link = XEXP (cur_link, 1))
			      {
				if (XEXP (cur_link, 0) == i2)
				  {
				    XEXP (cur_link, 0) = i3;							    
				    continue;
				  }
				if (XEXP (cur_link, 0) == i3)
				  XEXP (cur_link, 0) = i2;
			      }
			  }
		    }
		      ...
		      int in_dest)
{
  ...
  if (((GET_RTX_CLASS (code) == '2' || GET_RTX_CLASS (code) == 'c'
			 ...
			 == 'o'))))))
    {
      ...
      if (cond != 0
	  /* If cond is the same as x, then it is certainly not going to 
	     be any simpler to derive a flag from its value. This can
	     happen for a sign-bit extraction. */
	  && ! rtx_equal_p (x, cond)
