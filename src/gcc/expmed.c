 ...
 tryagain:
  if (HAVE_insv
      ...
      else
	{
	  ...
	  if (GET_CODE (xop0) == MEM && ! failed)
	    {
	      /* try again in registers */
	      failed = 1;
	      goto tryagain;
	    }
		       ...
		       unsigned HOST_WIDE_INT bitpos, rtx value)
{
  ...
  if (GET_CODE (value) != CONST_INT)
	...
	xop0 = op0;
      ...
      /* Compute OP0 ^= (OP0 ^ VALUE) & MASK.

         This uses two XORS and one AND, instead of two ANDS and one
         OR, but only needs one mask, and since it isn't complemented
         it is likely to be smaller and faster to load in most
         cases. */
      temp = expand_binop (mode, xor_optab, xop0, value,
			   NULL_RTX, 1, OPTAB_LIB_WIDEN);
      temp = expand_binop (mode, and_optab, temp, 
			   mask_rtx (mode, bitpos, bitsize, 0),
			   NULL_RTX, 1, OPTAB_LIB_WIDEN);
      temp = expand_binop (mode, xor_optab, xop0, temp,
	  ...
	  if (GET_CODE (xop0) == MEM)
	    {
	      ...
	      if (failed
		  || ! ((*insn_data[(int) CODE_FOR_extzv].operand[1].predicate)
			(xop0, GET_MODE (xop0))))
		{
	  ...
	  if (GET_CODE (xop0) == MEM)
	    {
	      /* Is the memory operand acceptable?  */
	      if (failed
		  || ! ((*insn_data[(int) CODE_FOR_extv].operand[1].predicate)
			 (xop0, GET_MODE (xop0))))
		{
		  /* No, load into a reg and extract from there.  */
		  enum machine_mode bestmode;
