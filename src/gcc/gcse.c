...
store_motion (void)
{
  ...
  /* Remove inserts that are going to be problematic for LOOP optimization. */
  for (ptr = first_ls_expr (); ptr != NULL; ptr = next_ls_expr (ptr))
    {
      for (x = 0; x < NUM_EDGES (edge_list); x++)
	if (TEST_BIT (pre_insert_map[x], ptr->index)) 
	  {
	    rtx first_insn = BB_HEAD (INDEX_EDGE (edge_list, x)->dest);
	    if (!first_insn)
	      continue;

	    /* check for loop edges, in which case, we don't really want to 
	       insert it here */	  
	    prev = PREV_INSN (first_insn);
	    while (prev && GET_CODE (prev))
	    {
	      if (NOTE_LINE_NUMBER (prev) == NOTE_INSN_LOOP_BEG)
		{
		  /* remove all edge insertions that match this pattern.
		     remove delete stored associated with this pattern. */
		  for (y = 0; y < NUM_EDGES (edge_list); y++)
		    RESET_BIT (pre_insert_map[y], ptr->index);	          
		  FOR_EACH_BB (bb)
		    RESET_BIT (pre_delete_map[bb->index], ptr->index);
		  break;
		}
	      prev = PREV_INSN (prev);
	    }
