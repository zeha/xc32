                ...
                char *use_addressof)
{
  ...
  else if (REGNO (reg) >= FIRST_PSEUDO_REGISTER && !DF_REF_CHAIN (ref))
    {
      /* Do not rename for undefined uses. */
      newreg = reg;
      if (rtl_dump_file)
	fprintf (rtl_dump_file,
		 "New web forced to keep reg=%i (use without define)\n",
		 REGNO (reg));
    }
  else
    {
...
web_main (void)
{
  ...
  for (insn = get_insns (); insn; insn = NEXT_INSN (insn))
	...
	if (targetm.web_mark_register)
	  (*targetm.web_mark_register) (insn, use_addressof);
      }
