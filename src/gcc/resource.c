...
{  
  ...
  if (b == -1)
    b = find_basic_block (insn, -1);

  if (target_hash_table != NULL && !tinfo) 
    {
      /* Should not happen, since an entry should of already be added.  
	 but allocate and chain it into the hash table.  */           
      tinfo = xmalloc (sizeof (struct target_info));
      tinfo->uid = INSN_UID (insn);
      tinfo->block = b;           
      tinfo->next = target_hash_table[INSN_UID (insn) % TARGET_HASH_PRIME];
      target_hash_table[INSN_UID (insn) % TARGET_HASH_PRIME] = tinfo;         
    }
