		  ...
		  struct loop_desc *desc)
{
  ...
  if (!rtx_equal_p (XEXP (set_src, 0), desc->var))
    {
      /* Do a quick check to see if expr0 is simple alias with desc->var,
	 ie. it is assigned to another register once in the loop. */
      if (GET_CODE (XEXP (set_src, 0)) != REG)
	return NULL;

      /* Find insn that modifies expr0.  */
      mod_insn = simple_increment_regs[REGNO (XEXP (set_src, 0))];
      if (!mod_insn)
	return NULL;
      mod_bb1 = BLOCK_FOR_INSN (mod_insn);

      /* Check that it is executed exactly once each iteration.  */
      if (!just_once_each_iteration_p (loop, mod_bb1))
	return NULL;

      set = single_set (mod_insn);
      if (!set)
	abort ();
      if (!rtx_equal_p (SET_DEST (set), XEXP (set_src, 0)))
	abort ();
      if (!rtx_equal_p (SET_SRC (set), desc->var))	
	return NULL;

      /* If it gets here, we have a simple alias of desc->var. 
	 To get the loop optimizer to peel correctly and pick the right 
	 register as the correct initial value, we pick this alias since 
	 desc->var depends on it and there must be an initial value 
	 for it. */
      desc->var = SET_DEST (set);
    }
