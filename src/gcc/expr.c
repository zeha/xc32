...
store_by_pieces (rtx to, unsigned HOST_WIDE_INT len,
		 rtx (*constfun) (void *, HOST_WIDE_INT, enum machine_mode),
		 void *constfundata, unsigned int align, int endp, int memsetp)
{
...
#ifdef DISABLE_POWER2_BIT_MASK_TEST
      tree new_fold = fold_single_bit_test (code == NE ? NE_EXPR : EQ_EXPR,
					    arg0, arg1, type);
      if (new_fold)
	return expand_expr (new_fold, target, VOIDmode, EXPAND_NORMAL);	
#else
