...
scan_loop (struct loop *loop, int flags)
{
  ...
  prescan_loop (loop);
  threshold = (loop_info->has_call || SMALL_REGISTER_CLASSES ? 1 : 2) 
	      * (1 + n_non_fixed_regs);
...
strength_reduce (struct loop *loop, int flags)
{
  ...
  int threshold = (loop_info->has_call || SMALL_REGISTER_CLASSES ? 1 : 2) 
		  * (3 + n_non_fixed_regs);
...
#ifndef AGGRESSIVE_COMBINE_GIVS
