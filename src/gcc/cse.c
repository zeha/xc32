...
fold_rtx (rtx x, rtx insn)
{
  ...
  switch (code)
	...
	return x;
#else
      if (flag_no_function_cse && CONSTANT_P (XEXP (XEXP (x, 0), 0)))
	return x;
#endif
      break;
...
cse_insn (rtx insn, rtx libcall_insn)
{
  ...
  for (i = 0; i < n_sets; i++)
      ...
      else if (do_not_record)
	{
	  ...
	  /* Just because a memory reference is volatile doesn't 
	     mean we can't optimise its address. */
	  if (GET_CODE (dest) == MEM)
	    {
	      rtx addr = XEXP (dest, 0);
	      /* Check that the address itself doesn't contain 
		 volatile components. */
	      do_not_record = 0;
	      (void) HASH (addr, GET_MODE (addr));
	      if (! do_not_record)
		mention_regs (addr);
	    }
