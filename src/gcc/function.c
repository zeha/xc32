...
#define PARM_BYTES (PARM_BOUNDARY / BITS_PER_UNIT)
...
assign_parms (tree fndecl)
{
  ...
  for (parm = fnargs; parm; parm = TREE_CHAIN (parm))
      ...
      if (entry_parm)
	{
	  ...
	  if (partial
		...
		= CEIL_ROUND (pretend_bytes, STACK_BOUNDARY / BITS_PER_UNIT);
	  ...
	  && XVECLEN (entry_parm, 0) > 1)
	{
	  ...
	  /* Backported fix from gcc-4.0, see 
	     http://gcc.gnu.org/ml/gcc-patches/2004-09/msg01735.html */
	  /* Deal with register arguments that are padded at the least
	     significant end.  */
	  || (REG_P (entry_parm)
	      && GET_MODE_SIZE (promoted_mode) < UNITS_PER_WORD
	      && (BLOCK_REG_PADDING (passed_mode, passed_type, 1)
		  == (BYTES_BIG_ENDIAN ? upward : downward)))
	  /* End backported fix from gcc-4.0 */
#endif
	  || GET_CODE (entry_parm) == PARALLEL)
	{
  ...
  current_function_args_size
    = ((current_function_args_size + PARM_BYTES - 1)
       / PARM_BYTES) * PARM_BYTES;
  ...
  for (arg = DECL_ARGUMENTS (fndecl); arg;
	...
	&& (TYPE_MODE (DECL_ARG_TYPE (arg)) == TYPE_MODE (TREE_TYPE (arg))
	    /* Handle prototype promotion too, if this function has a
	       ...
	       arguments now generates an error, so this is safe. */
	    || (flag_unit_at_a_time
		&& ! TREE_PUBLIC (fndecl)
...
expand_dummy_function_end (void)
{
  ...
  /* Compensate for increment in allocate_struct_function(). */
  funcdef_no--;
}
