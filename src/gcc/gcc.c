...
%eGNU C no longer supports -traditional without -E}\
  ...
  {".sx", "@assembler-with-cpp", 0},
#endif
  {".S", "@assembler-with-cpp", 0},
  {"@assembler-with-cpp",
#ifdef AS_NEEDS_DASH_FOR_PIPED_INPUT
   "%(trad_capable_cpp) %(cpp_options)\
      %{E|M|MM:%(cpp_debug_options)} -lang-asm\
      %{!M:%{!MM:%{!E:%{!S:-o %|.s |\n\
...
grow_arg (const char *arg, int delete_always, int delete_failure)
{
  ...
  string = xmalloc (strlen (arg) + strlen (argbuf[argbuf_index - 1]) + 1);
...
/* This is a non-minGW fix to the Vista access issue GCC Bug 30972 */
