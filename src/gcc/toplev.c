...
/* Enables the use of section relative addressing. This option
   requires unit-at-a-time */

int flag_section_relative_addressing = 0;
...
static const lang_independent_options f_options[] =
{
  ...
  {"section-relative-addressing", &flag_section_relative_addressing, 1 },
...
/* Reinitialize the compiler back end when register classes or
   instruction sets have changed, before each function.  */
void
backend_reinit (void)
{
  init_emit_regs ();
  ...
  /* The following initialization functions need to generate rtl, so
     provide a dummy function context for them, if needed. */
  init_dummy_function_start ();
  init_expr_once ();
  init_expmed ();
  if (flag_caller_saves)
