...
struct comp_unit
{
  ...
  /* Whether addresses should be sign-extended. */
  unsigned char sign_extend_vma;
};
...
{
  if (unit->sign_extend_vma == 1)
    {
      switch (unit->addr_size)
	{
	case 8:
	  return bfd_get_signed_64 (unit->abfd, (bfd_byte *) buf);
	case 4:
	  return bfd_get_signed_32 (unit->abfd, (bfd_byte *) buf);
	case 2:
	  return bfd_get_signed_16 (unit->abfd, (bfd_byte *) buf);
	default:
	  abort ();
	}
    }
		 ...
		 unsigned int offset_size)
{
  ...
  /* Cache the result of bfd_get_sign_extend_vma. */
  unit->sign_extend_vma = bfd_get_sign_extend_vma (abfd);
