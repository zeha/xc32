...
struct mips_elf_link_hash_entry
{
  ...
  bfd_boolean maybe_need_fn_stub;
...
#define FN_STUB_P(s) (strncmp (s, FN_STUB, sizeof FN_STUB - 1) == 0)
#define CALL_STUB_P(s) (strncmp (s, CALL_STUB, sizeof CALL_STUB - 1) == 0)
#define CALL_FP_STUB_P(s) (strncmp (s, CALL_FP_STUB, sizeof CALL_FP_STUB - 1) == 0)

			     ...
			     void *data ATTRIBUTE_UNUSED)
{
  ...
  if (h->fn_stub != NULL
      && !h->maybe_need_fn_stub)
    {
      /* We don't need the fn_stub; the only references to this symbol
         are 16 bit calls.  Clobber the size to 0 to prevent it from
         being included in the link.  */
      h->fn_stub->size = 0;
      h->fn_stub->flags &= ~(SEC_RELOC | SEC_KEEP);
  ...
  if (h->call_stub != NULL
      && (h->root.other & STO_MIPS16))
    {
      ...
      h->call_stub->size = 0;
      h->call_stub->flags &= ~(SEC_RELOC | SEC_KEEP);
  ...
  if (h->call_fp_stub != NULL
      && (h->root.other & STO_MIPS16))
    {
      /* We don't need the call_fp_stub; this is a 16 bit function, so
         calls from other 16 bit functions are OK.  Clobber the size
         to 0 to prevent it from being included in the link.  */
      h->call_fp_stub->size = 0;
      h->call_fp_stub->flags &= ~(SEC_RELOC | SEC_KEEP);
...
/* Return whether a relocation is against a local symbol in a link-once 
   section.  */

static bfd_boolean
mips_elf_linkonce_relocation_p (bfd *input_bfd,
				const Elf_Internal_Rela *rel,
				asection **local_sections)
{
  unsigned long r_symndx = ELF_R_SYM (input_bfd, rel->r_info);
  asection * sec = local_sections[r_symndx];
  return sec && (sec->flags & SEC_LINK_ONCE) != 0;
}
			       ...
			       bfd_boolean save_addend)
{
  ...
  /* If this is a 32- or 64-bit call to a 16-bit function with a stub, we
     ...
     a stub.  */
  if (r_type == R_MIPS_26 && !info->relocatable
	...
	{
	  BFD_ASSERT (h->maybe_need_fn_stub);
	  sec = h->fn_stub;
	}
  ...
  /* If this is a 16-bit call to a 32- or 64-bit function with a stub, we
     need to redirect the call to the stub.  */
	   ...
	   && ((h != NULL && (h->call_stub != NULL || h->call_fp_stub != NULL))
	       || (local_p && elf_tdata (input_bfd)->local_call_stubs != NULL
  ...
  *require_jalxp = (!info->relocatable
  ...
  switch (r_type)
    ...
    case R_MIPS_PC16:
      value = _bfd_mips_elf_sign_extend (addend, 18) + symbol - p;
      overflowed_p = mips_elf_overflow_p (value, 18);
...
#define MIPS_READONLY_SECTION (SEC_ALLOC | SEC_LOAD | SEC_READONLY)
	...
	case R_MIPS_JALR:
      ...
      /* If this reloc is not a 16 bit call, and it has a global symbol,
	 then we will need the fn_stub if there is one, if it is actually
	 used by a call.  References from a stub section do not count.  */
      if (h != NULL
	  && r_type == R_MIPS_26
				...
				asection **local_sections)
{
  ...
  const struct elf_backend_data *bed;
  const Elf_Internal_Rela *hi16_relocation = NULL;
		 ...
		 relocations.   */
		  ...
		  || (r_type == R_MIPS_GOT16
		      && mips_elf_local_relocation_p (input_bfd, rel,
						      local_sections, FALSE)
		      && ! mips_elf_linkonce_relocation_p (input_bfd, rel,
							   local_sections)))
		{
		  bfd_vma l;
		  ...
		  if (lo16_relocation == NULL)
			...
			(info, _("unmatched HI16 relocation"), name,
			 input_bfd, input_section, rel->r_offset);

		      return FALSE; */

		      /* HACK: Ignore %HI relocation that don't have a 
			 matching %LO relocation. */
		      continue;
		    }
		  ...
		  addend <<= 16;
		  /* Record the addend for use with corresponding R_MIPS_LO16 
		     relocation. Because all %hi %lo %lo are chained, the next 
		     n number of %lo will use this as its hi part, but with 
		     its own offset. */
		  hi16_addend = addend;
		  ...
		  /* Record HI relocation position. */
		  hi16_relocation = rel;
		}
	      else if (r_type == R_MIPS_LO16 || r_type == R_MIPS16_LO16)
		{
		  /* Check if a previous matching GOT16 or HI16 had setup an 
		     addend that this relocation can use. We need the full
		     32-bit addend for section merging lookup. */
		  if (hi16_relocation
		      && (ELF_R_SYM (input_bfd, hi16_relocation->r_info) == 
			  ELF_R_SYM (input_bfd, rel->r_info)))
		    addend += hi16_addend;
		  else
		    {
		      /* No matching hi16, is this a fatal error? */
		      /* For now, just use the addend from the instruction */
		      hi16_relocation = NULL;
...
#ifdef EF_MIPS_SDE_FP64
#define EF_MIPS_FP64_FLAGS	(EF_MIPS_FP64 | EF_MIPS_SDE_FP64)
#else
#define EF_MIPS_FP64_FLAGS	(EF_MIPS_FP64)
#endif

static bfd_boolean
mips_fp64_flags_p (flagword flags)
{
  return (! mips_32bit_flags_p (flags)
	  || (flags & EF_MIPS_FP64_FLAGS));
}
...
_bfd_mips_elf_merge_private_bfd_data (bfd *ibfd, bfd *obfd)
{
  ...
  if (mips_32bit_flags_p (old_flags) != mips_32bit_flags_p (new_flags))
  ...
  else if (mips_fp64_flags_p (old_flags) != mips_fp64_flags_p (new_flags))
    {
      bfd_boolean uses_fpu = FALSE;

      /* Don't complain if this module doesn't use floating-point. */
      if ((sec = bfd_get_section_by_name (ibfd, ".reginfo")))
	{
	  Elf32_External_RegInfo ext;
	  Elf32_RegInfo ri;
	  if (bfd_get_section_contents (ibfd, sec, (PTR) &ext,
					(file_ptr) 0, sizeof ext))
	    {
	      bfd_mips_elf32_swap_reginfo_in (ibfd, &ext, &ri);
	      uses_fpu = ri.ri_cprmask[1] != 0;
	    }
	}
	...
	{
	  (*_bfd_error_handler)
	    (_("%B: linking 32-bit FPU code with 64-bit FPU code"),
	     ibfd);
	  ok = FALSE;
	}

      elf_elfheader (obfd)->e_flags |= new_flags & EF_MIPS_FP64_FLAGS;

      new_flags &= ~ EF_MIPS_FP64_FLAGS;
      old_flags &= ~ EF_MIPS_FP64_FLAGS;
    }
...
_bfd_mips_elf_print_private_bfd_data (bfd *abfd, void *ptr)
{
  ...
  else if (elf_elfheader (abfd)->e_flags & EF_MIPS_FP64_FLAGS)
    fprintf (file, _(" [64bit floats]"));
  else
				      ...
				      bfd_boolean dynamic)
{
  if (!dynamic && definition
    ...
    {
      /* Inherit st_other processor-specific field from the actual
	 symbol definition, overriding a weak definition. */
      unsigned char other = isym->st_other;
