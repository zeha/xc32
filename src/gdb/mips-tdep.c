...
enum
{ NUM_MIPS_PROCESSOR_REGS = (LAST_EMBED_REGNUM + 1 - 32) };
...
#define MIPS32_COP0(reg,sel) (CP0_REGNUM+32*(sel)+(reg))
...
static const struct mips32_reg mips32_dsp[] = {
  ...
  {DSPACC_REGNUM+1,	"hi1"},
  {DSPACC_REGNUM+2,	"lo1"},
  /* {DSPACC_REGNUM+3,	"acx1"}, */
  {DSPACC_REGNUM+4,	"hi2"},
  {DSPACC_REGNUM+5,	"lo2"},
  /* {DSPACC_REGNUM+6,	"acx2"}, */
  {DSPACC_REGNUM+7,	"hi3"},
  {DSPACC_REGNUM+8,	"lo3"},
			  ...
			  struct reggroup *reggroup)
{
  ...
  if (reggroup == save_reggroup || reggroup == restore_reggroup)
    return pseudo && raw_p 
	  ...
	  || rawnum == CP0_REGNUM + 1	/* cp0.random */
	  || rawnum == CP0_REGNUM + 9	/* cp0.count */);
  return 0;
}
...
mips_unwind_pc (struct gdbarch *gdbarch, struct frame_info *next_frame)
{
  ...
  if (frame_relative_level(next_frame) >= 0)
	...
	pc = frame_unwind_register_signed (next_frame, RA_REGNUM);
      if (mips_in_frame_stub(pc))
	pc = frame_unwind_register_signed (next_frame, S2_REGNUM);
    }
  return pc;
}
...
{				/* do values for FP (float) regs */
  ...
  raw_buffer =
    (char *) alloca (2 * mips_float_regsize (current_gdbarch));
  ...
  if (regnum % 2 == 0
	...
	internal_error (__FILE__, __LINE__,
			"mips_print_fp_register: bad access to "
			"64-bit FP register");
...
mips16_instruction_has_delay_slot (CORE_ADDR addr, int mustbe32)
{
  ...
  if (!mustbe32)
    return (inst & 0xf89f) == 0xe800;		/* jr/jalr (16bit instruction) */
  return (inst & 0xf800) == 0x1800;		/* jal/jalx (32bit instruction) */
}
  ...
  if (strncmp (name, mips_str_mips16_call_stub,
	       ...
	       || name[strlen (mips_str_mips16_call_stub)] == 'd')
	{
	  if (pc == start_addr)
		...
		{
		  t_inst inst = mips_fetch_instruction (target_pc);
		  CORE_ADDR addr = inst;
...
mips_ignore_helper (CORE_ADDR pc)
{
  ...
  return (strcmp (name, mips_str_mips16_ret_sf) == 0
	  || strcmp (name, mips_str_mips16_ret_df) == 0);
}
