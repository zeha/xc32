...
warn_deprecated_use (tree node, tree attr)
{
  ...
  int unsafe = 0; /* MERGE-NOTES: this is for the xc16 'unsafe' attribute. */
#endif
...
#ifdef _BUILD_MCHP_
	      unsupported
		= lookup_attribute ("unsupported",
				    TYPE_ATTRIBUTES (TREE_TYPE (decl)));
	      if (!unsupported)
		unsupported
		  = lookup_attribute ("__unsupported__",
				      TYPE_ATTRIBUTES (TREE_TYPE (decl)));
	      deprecated = lookup_attribute ("deprecated", attr);
	      as_error = lookup_attribute ("target_error", attr);
	      if (!as_error)
...
/* MERGE-NOTES: not ported from xc32/tree.c/warn_deprecated_use */
  if (attr)
    {
#ifdef _BUILD_MCHP_
      ...
      if (as_error)
	{
      ...
      if (unsupported)
	{
	  // NOTE: wouldn't it be better to lose the " is unsupported" part
	  // here? (the message already contains this syntagm)
