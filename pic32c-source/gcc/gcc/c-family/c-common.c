				  ...
				  unsigned HOST_WIDE_INT param_num)
{
  if (CONVERT_EXPR_P (param)
    ...
    {
      /* Strip coercion.  */
      /* MERGE-NOTES: check third parameter */
...
#if defined(_BUILD_MCHP_) && defined(TARGET_MCHP_PIC32C)
void
check_function_arguments_recurse_update (void (*callback)
				          (void *, tree *, unsigned HOST_WIDE_INT),
				         void *ctx, 
                                         tree *param_p,
				         unsigned HOST_WIDE_INT param_num)
{
  ...
  if (CONVERT_EXPR_P (param)
    ...
    {
      /* Strip coercion.  */
      check_function_arguments_recurse_update (callback, ctx,
					       &TREE_OPERAND (param, 0),
					       param_num);
