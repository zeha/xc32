...
extern bool callee_saved_reg_p(unsigned int reg);
...
public:
  ...
  void set_pass_param (unsigned int n, bool param)
    {
      gcc_assert (n == 0);
      remove_redundant_psh_pop_insn = param;
    }

private:
  bool remove_redundant_psh_pop_insn;
...
pass_cprop_hardreg::execute (function *fun)
{
  ...
  // Have this optimization enabled just for ARM
  if (remove_redundant_psh_pop_insn)
    return 0;
#endif
  ...
  if (remove_redundant_psh_pop_insn)
  {
    ...
    run_fast_dce ();
    changed = false;
    for (reg = 0; reg <= LAST_HI_REGNUM; reg++)
      {
        if (df_regs_ever_live_p (reg) && callee_saved_reg_p (reg))
          {
            if ((df->use_regs[reg]->n_refs == 0)
                && (df->def_regs[reg]->n_refs == 0))
              {
                df_set_regs_ever_live (reg, false);
                changed = true;
              }
          }
      }
