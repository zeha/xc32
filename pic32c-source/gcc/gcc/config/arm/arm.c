...
/* MERGE_TODO: why exclude this function? */
#ifndef _BUILD_MCHP_
...
#ifdef _BUILD_MCHP_
#ifdef PIC32C_SUBTARGET_ATTRIBUTE_TABLE
    PIC32C_SUBTARGET_ATTRIBUTE_TABLE
#endif
#endif
			    ...
			    bool warn_compatible)
{
  ...
  /* If the user did not specify a processor or architecture, choose
  ...
  else
    {
      ...
      /* MERGE-NOTES: this is using TARGET_CPU_DEFAULT as if it were a string,
         where other uses in this file are (correctly) as an enum. 
         I've tentatively modified this to instead use the name in the
         cpu table indexed by the target default.

         Also, there is now support for setting the default arch, cpu, fpu,
         etc. using --with options in the configuration. We should consider
         that approach, though we'll have to ensure the defaults
         match our MULTILIB_DEFAULT in pic32c.h. 
         */
      arm_selected_cpu = arm_parse_cpu_option_name (all_cores, "default CPU",
						    all_cores[TARGET_CPU_DEFAULT].common.name);
      cpu_opts = strchr (all_cores[TARGET_CPU_DEFAULT].common.name, '+');
...
arm_option_override (void)
{
  ...
  /* MERGE_TODO: move this to SUBTARGET_OVERRIDE_OPTIONS? */
#ifdef _BUILD_MCHP_
      ...
      switch (code)
        ...
        case COMPARE:
          ...
          if (REG_P (XEXP (x, 0)) && CONST_INT_P (XEXP (x, 1)))
            {
              // check if const is in range 0-255
              if (satisfies_constraint_Py (XEXP (x, 1)))
                *cost = COSTS_N_INSNS (1);
              // check if const has to be split or not
              else if (const_ok_for_op (INTVAL (XEXP (x, 1)), code))
                *cost = COSTS_N_INSNS (2);
              else
        ...
        case ASHIFTRT:
        case LSHIFTRT:
          *cost = (mode == SImode) ? COSTS_N_INSNS (1) : COSTS_N_INSNS (2);
          return true;
        ...
        case SET:
          ...
          if (REG_P (XEXP (x, 0)) && CONST_INT_P (XEXP (x, 1)))
            {
              if (satisfies_constraint_Py (SET_SRC (x)))
                *cost = COSTS_N_INSNS (1);
              else
                *cost = COSTS_N_INSNS (2);
              return true;
          ...
          // Load from memory
          else if (MEM_P (XEXP (x, 1)))
            {
              rtx load_from = XEXP (x, 1);

              if (REG_P (XEXP (load_from, 0)))
                {
                  *cost = COSTS_N_INSNS (1);
                  return true;
                }
              else if ((GET_CODE (XEXP (load_from, 0)) == PLUS))
                {
                  rtx plus = XEXP (load_from, 0);
                  if (CONST_INT_P (XEXP (plus, 1)))
                    {
                      HOST_WIDE_INT cint = INTVAL (XEXP (plus, 1));
                      if (((cint & 0x3) == 0) && (cint >= 0) && (cint <= 124))
                        *cost = COSTS_N_INSNS (1);
                      else
                        *cost = COSTS_N_INSNS (2);
                      return true;
          ...
          // Store to memory
            ...
            {
              rtx store_to = XEXP (x, 0);

              if (REG_P (XEXP (store_to, 0)))
                {
                  *cost = COSTS_N_INSNS (1);
                  return true;
                }
              else if ((GET_CODE (XEXP (store_to, 0)) == PLUS))
                {
                  rtx plus = XEXP (store_to, 0);
                  if (CONST_INT_P (XEXP (plus, 1)))
                    {
                      HOST_WIDE_INT cint = INTVAL (XEXP (plus, 1));
                      if (((cint & 0x3) == 0) && (cint >= 0) && (cint <= 124))
  ...
  while (in_words_to_go >= 2)
    {
      ...
      const int force_write_back = optimize_size && !first_chunk;
      first_chunk = 0;
#endif
      if (in_words_to_go > 4)
...
#ifdef TARGET_MCHP_PIC32C
bool
callee_saved_reg_p (unsigned int reg)
{
  return (!call_used_regs[reg]
          || (TARGET_THUMB1 && optimize_size && reg >= FIRST_HI_REGNUM
              && reg <= LAST_HI_REGNUM));
}
#else
...
#ifdef _BUILD_MCHP_
  save_reg_mask = SUBTARGET_INIT_SAVE_REG_MASK;
#endif
...
thumb1_compute_save_core_reg_mask (void)
{
  ...
  unsigned reg;
#ifdef _BUILD_MCHP_
  mask = SUBTARGET_INIT_SAVE_REG_MASK;
#else
  mask = 0;
#endif
...
#ifdef TARGET_MCHP_PIC32C
  /* Adjust stack usage estimation for functions with where
   * prologue will contain a push instruction which is not
   * taken into account by the stack usage:
   *    @ args = 4, pretend = 16, frame = 0
   *    @ frame_needed = 1, uses_anonymous_args = 1
   *    push    {r0, r1, r2, r3}        @
   */
  if (mchp_stack_usage)
    current_function_static_stack_size += crtl->args.pretend_args_size;
#endif
  ...
  if (priority != DEFAULT_INIT_PRIORITY)
    {
      ...
      sprintf (buf, "%s.%.5u",
	       ...
	       priority);
/* MERGE-NOTES: .init_array et al are mapped to flash. SECTION_WRITE must be some
   kind of weird handshake here. Not yet clear if we should just force this
   with 'implied attributes' in the linker, because this ends up as 
   .section .init_array.priority, data
   which is not kosher for flash.
   */
#ifdef TARGET_MCHP_PIC32C
      s = get_section (buf, SECTION_CODE | SECTION_NOTYPE, NULL_TREE);
#else
...
#ifdef _BUILD_MCHP_
#ifdef SUBTARGET_ASM_FILE_END /* MCHP PIC32C */
  SUBTARGET_ASM_FILE_END();
#endif
#endif
...
#ifdef _BUILD_MCHP_
#ifdef PIC32C_SUBTARGET_ENCODE_SECTION_INFO
