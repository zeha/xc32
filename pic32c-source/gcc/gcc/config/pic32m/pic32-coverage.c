  ...
  if (!flag_pic)
    {
      if (!TARGET_MIPS16)
      ...
      else
        {
          ...
          emit_insn (gen_rtx_SET (CC_ADDR_REG, cc_bits_ref));
        }
    }
  else
    {
      ...
      * so use reg $2 and possibly $3 instead, but save their values */
          ...
          if (CC_TMP1_REGNO > 7)
            {
              ...
              emit_insn (gen_rtx_SET (CC_SAVE1_REG, CC_TMP1_REG));
            }
          if (CC_TMP2_REGNO > 7)
            {
              CC_SAVE2_REG = CC_TMP2_REG;
              CC_TMP2_REG = gen_rtx_REG (SImode, save_regno);
              emit_insn (gen_rtx_SET (CC_SAVE2_REG, CC_TMP2_REG));
            }
        }
      ...
      if (!TARGET_MIPS16)
      ...
      else
        {
          ...
          emit_insn (gen_rtx_SET (CC_TMP2_REG, GEN_INT (bit_mask)));
      ...
      if (CC_SAVE1_REG != NULL_RTX)
        emit_insn (gen_rtx_SET (CC_TMP1_REG, CC_SAVE1_REG));

      if (CC_SAVE2_REG != NULL_RTX)
        emit_insn (gen_rtx_SET (CC_TMP2_REG, CC_SAVE2_REG));
    }
  else
    {
