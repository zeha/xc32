...
/* MERGE-TODO: hopefully redundant. */
...
# define STARTFILE_SPEC " \
  ...
  %{!A:%{!nostdlib:%{!mno-default-isr-vectors:%{!mdebugger : %{!mreserve=*: -l:debug-exception-return.o} }}}} \
 "
...
%{!fsyntax-only:%{!c:%{!M:%{!MM:%{!E:%{!S:\
   ...
   "%X %{o*} %{e*} %{N} %{n} %{r}\
    ...
    %{fprofile-arcs|fprofile-generate*|coverage:-lgcov} " SANITIZER_SPEC " \
        %{mprocessor=*: %{!A:%{!nostdlib:%{!nodefaultlibs:%{!nostartfiles:%{!mno-default-isr-vectors:%{!mdfp=*: -l:default_isr_vectors.o} }}}}}}\
        %{!A:%{!nostdlib:%{!nodefaultlibs:%{!nostartfiles:%{!mno-default-isr-vectors: -l:pic32_software_reset.o} }}}}\
        %{mreserve=*:--mreserve=%* } \
        %{mdfp=*: %{mprocessor=*: -D__XC32_WITH_DFP=1}} \
    %{!nostdlib:%{!nodefaultlibs:%(link_ssp) %(link_gcc_c_sequence)}}\
    %{mchp-stack-usage: --mchp-stack-usage} \
    %{!nostdlib:%{!nostartfiles:%E}}" 
#define _LINK_COMMAND_END "\
    \n%(post_link) }}}}}}"

/* 'normal' link command spec which forces a default script if mprocessor is
   used. */
#undef LINK_COMMAND_SPEC
#define LINK_COMMAND_SPEC \
  _LINK_COMMAND_START \
  "%{T:%{T*}; mprocessor=*:-T %s%J%{mdfp=*:%*/xc32/;:proc/}%J%{mprocessor=*:%*}/%J%{mprocessor=*:p%*}%J%{fpie|fPIC:_pic}%J.ld}" \
  _LINK_COMMAND_END

/* same, without any default script added. */
...
#define LINK_LIBGCC_SPEC "\
 %D %{!mdfp=* : -L %s%{mprocessor=*:./proc/%*; :./proc/32MXGENERIC}} %{mdebugger:%{!mreserve=data*:--defsym _DEBUGGER=1}}"
 ...
 * of the options utilized in that spec.
 *
 * NB: FP_ASM_SPEC is defined in mips.h conditional on HAVE_AS_DOT_MODULE,
 * and is responsible for passing soft|hard-float to the assembler. 
 */
/* XC32-1261: the mips16e option is long gone from gcc but lives on in our
   specs. as the assembler no longer accepts it, we handle it as in CC1_SPEC for
   the assembler.
   */
#undef ASM_SPEC
#define ASM_SPEC "\
...
/* Common spec functions from mchp-cci */
extern const char *mchp_last_of (int, const char **);

/* We add some extra spec functions here. The MIPS target doesn't define this
   at the time of writing, so we ensure we get an error if that changes. */
#ifdef EXTRA_SPEC_FUNCTIONS
#error "EXTRA_SPEC_FUNCTIONS defined, see remarks in source"
#else
#define EXTRA_SPEC_FUNCTIONS { "last_of", mchp_last_of },
#endif

/* A spec that infers the -mdsp setting from an -march argument.  */
/* MERGE-NOTES: consider cleaning up no|hard|soft-float. these specs
   as is would emit msoft-float even if mhard-float was specified. */
#undef BASE_DRIVER_SELF_SPECS
#define BASE_DRIVER_SELF_SPECS "\
   %{mprocessor=*: -mprocessor=%:last_of(-mprocessor %{mprocessor=*: %*} %<mprocessor=*)} \
   ...
   %{!r:%{mprocessor=*: %{!c:%{!S:%{!E:%{!A:%{!nostdlib:%{!nodefaultlibs:%{!nostartfiles:%{!mno-default-isr-vectors:%{mdfp=*: %* %J%{mprocessor=*:/xc32/%*/p%*_div.S } ;: %J%{mprocessor=*: %s./proc/%*/p%*_div.S } }}}}}}}}} }}\
  "
...
/* XC32-1522: while correct, using long may cause incompatible pointer type
   warnings with other 32-bit types. */
#undef SIZE_TYPE
    ...
    if (TARGET_ABICALLS || flag_pic)                        \
      {                                                     \
        ...
        builtin_define ("__pic__");                         \
                                                            ...
                                                            \
    if (mips_abi != ABI_32)                                 \
      fatal_error (__LINE__, "internal error: mips_abi != ABI_32");   \
             ...
             {                                                        \
              ...
              snprintf (mchp_it_define, strlen ("__IT_TRANSPORT= ") + \
               ...
               {                                                      \
                mchp_it_define =(char *)                              \
                alloca (strlen ("__IT_TRANSPORT=xx") +                \
                      strlen(mchp_it_transport) + 1);                 \
                snprintf (mchp_it_define, strlen ("__IT_TRANSPORT= ") +   \
                      strlen(mchp_it_transport) + 1, "__IT_TRANSPORT=%s", \
                      mchp_it_transport);                             \
               }                                                      \
...
#define MIPS_SUBTARGET_OUTPUT_FUNCTION_EPILOGUE(file) \
  mchp_output_function_epilogue(file)
...
#define MIPS_SUBTARGET_ATTRIBUTE_TABLE                                                        \
    ...
    { "interrupt",        0, 1,  false, true,  true, false, mchp_interrupt_attribute, NULL},        \
    { "vector",           1, 256, true,  false, false, false, mchp_vector_attribute, NULL },        \
    { "at_vector",        1, 1,   true,  false, false, false, mchp_at_vector_attribute, NULL },     \
    { "nomicromips",      0, 0,   true,  false, false, false, mchp_nomicromips_attribute, NULL },   \
    /* also allow functions to be created without prologue/epilogue code */                   \
    { "naked",            0, 0,  true,  false, false, false, mchp_naked_attribute, NULL },          \
    { "address",          1, 1,  false, false, false, false, mchp_address_attribute, NULL },        \
    { "space",            1, 1,  false, false, false, false, mchp_space_attribute, NULL },          \
    { "persistent",       0, 0,  false, false, false, false, mchp_persistent_attribute, NULL },     \
    { "ramfunc",          0, 0,  false, true,  true, false,  mchp_ramfunc_attribute, NULL },        \
    { "unsupported",      0, 1,  false, false, false, false, mchp_unsupported_attribute, NULL },    \
    { "target_error",     1, 1,  false, false, false, false, mchp_target_error_attribute, NULL },   \
    { "keep",             0, 0,  false, false, false, false, mchp_keep_attribute, NULL },           \
    { "coherent",         0, 0,  false, false, false, false, mchp_coherent_attribute, NULL },       \
    { "crypto",           0, 0,  false, false, false, false, mchp_crypto_attribute, NULL },	      \
    { "unique_section",   0, 0,  true,  false, false, false, mchp_unique_section_attribute, NULL }, \
    { "region",           1, 1,  false, false, false, false, mchp_region_attribute, NULL },         \
...
#define MIPS_SUBTARGET_OUTPUT_FUNCTION_PROLOGUE(file) \
  mchp_output_function_prologue(file)
...
#define GCC_DRIVER_HOST_INITIALIZATION  \
    {                                                       \
        ...
        /* if no -mdfp=, use the default/internal path */                          \
        if (path == NULL)                                                          \
          path = make_relative_prefix(decoded_options[0].arg, "/bin/bin", "/bin"); \
        for (index=1; index < decoded_options_count; index++) {                               \
...
/* MCHP stack-usage */
#define TARGET_STACK_USAGE_SECTION  pic32_emit_su_section
