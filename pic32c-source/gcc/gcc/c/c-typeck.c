			 ...
			 vec<tree, va_gc> *origtypes)
{
  ...
  if (fundecl
    ...
    {
      if (!check_builtin_function_arguments (loc, arg_loc, fundecl, nargs, argarray))
        return error_mark_node;
      ...
      switch (DECL_FUNCTION_CODE (fundecl))
        ...
        case BUILT_IN_VSNPRINTF:
          /* DECL_IS_BUILTIN || !DECL_INITIAL ensures the declaration is that supplied 
             by the front-end or standard headers and not a user declaration. 
             DECL_ASSEMBLER_NAME_SET_P ensures the user has not modified the assembler 
             name of the builtin. */
	      if (TARGET_MCHP_SMARTIO_ENABLED
	          && (DECL_IS_BUILTIN (fundecl) || !DECL_INITIAL (fundecl))
	          && !DECL_ASSEMBLER_NAME_SET_P (fundecl))
	        {
              annot_stdio = 1;
            }
          break;
