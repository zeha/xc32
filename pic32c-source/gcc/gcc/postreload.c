...
/*
  x   - value that is already stored in register
  y   - value to be stored in register

  nb  - number of non-zero bits

  // ??? what about arithmetic-shift-right that inserts non-zero bits ???
  if (nb(x) < nb(y))
    return;
  ...
  if (nb(x) == nb(y)) {
    ...
    try ror(x,n)
    //may or may not be possible to express y as ror(x,n)
    //x = 1001
    //y = 1100
  }
  else {
...
*/
/* Convert move insns with constant inputs to shift/rotate if they are cheaper.
   Return true if any changes were made.  */
static bool
reload_cse_move2shift (rtx_insn *insn)
{
  rtx pat = PATTERN (insn);

  if (GET_CODE (pat) == SET && REG_P (SET_DEST (pat))
      && CONST_INT_P (SET_SRC (pat)) && GET_MODE (SET_DEST (pat)) == SImode)
    {

      rtx shift_op = NULL;
      rtx const_op = SET_SRC (pat);
      rtx reg_op = SET_DEST (pat);
      int regno = REGNO (reg_op);

      // const values
      uint32_t x = (uint32_t)reg_offset[regno];
      uint32_t y = (uint32_t)UINTVAL (const_op);
      // number of non-zero bits
      int nbx = __builtin_popcount (x);
      int nby = __builtin_popcount (y);
      // low-bit
      int lbx = ffs (x);
      int lby = ffs (y);
      // nr of positions to shift/rotate
      int pos = 0;

      // Check if the instruction to be optimized is "MOVS <Rd>,#<imm8>"
      //  If this is the case the instruction is already encoded as 16-bit
      //  Thumb instruction
      if (y <= 255)
        return false;
      ...
      if (nbx == nby)
        {
          if (nbx == 1)
            {
              if (x < y)
                shift_op = gen_rtx_ASHIFT (SImode, reg_op,
                                           gen_int_mode ((lby - lbx), SImode));
              else
                shift_op = gen_rtx_LSHIFTRT (
                    SImode, reg_op, gen_int_mode ((lbx - lby), SImode));
            }
          else
            {
              // try to rotate-right
              for (pos = 0; (pos != 32) && (x != y); pos++)
                x = (x >> 1) | (x << 31);
              if (pos != 32)
                {
                  gcc_assert ((pos >= 0) && (pos <= 31));
                  shift_op = gen_rtx_ROTATERT (SImode, reg_op,
                                               gen_int_mode (pos, SImode));
                }
            }
        }
      else
          ...
          if (x < y)
          ...
          else
            {
              ...
              if (x)
                {
                  gcc_assert ((pos >= 0) && (pos <= 31));
                  shift_op = gen_rtx_LSHIFTRT (SImode, reg_op,
                                               gen_int_mode (pos, SImode));
                }
            }
        }
      ...
      if (shift_op)
        {
          ...
          get_full_set_rtx_cost (pat, &oldcst);
          get_full_set_rtx_cost (shift_op, &newcst);
          if (costs_lt_p (&newcst, &oldcst, speed)
              && validate_change (insn, &SET_SRC (pat), shift_op, 0))
            {
              move2add_record_sym_value (SET_DEST (pat), NULL_RTX, const_op);
              return true;
            }
        }
    }
