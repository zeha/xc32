...
{
  rtx dest_rtx, seq, x;
  machine_mode dest_mode, src_mode, tmp_mode;
  int unsignedp;
  ...
  tmp_mode = promote_decl_mode(name, &unsignedp); // \mgeanta fix for sign-extension issue
  gcc_assert (src_mode == TYPE_MODE (TREE_TYPE (name)));
  /* MERGE-NOTES: this ICEs building newlib on arm. */
