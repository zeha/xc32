...
thread_prologue_and_epilogue_insns (void)
{
  ...
  if (epilogue_seq)
    {
      /*
        Look for basic blocks within the epilogue insns.
        This is similar to what is already implemented for prologue BB:
        ...
        We need this additional step because the MIPS arch inserts jump insns
        during prologue / epilogue generation (when handling the
        __attribute__((interrupt(IPL2AUTO)))), and without calling
        find_many_sub_basic_blocks we might end up with epilogue BB having
        multiple jump insnsn: this will result in an incorrect CFG that
        offends the scheduler. Below is an example that would lead to an
        infinite loop while the sched2 pass is executed:

        void __attribute__((interrupt(IPL2AUTO))) isr3(void)
        {
          if (t0)
            foo();
        }
      ...
      */
      if (BLOCK_FOR_INSN (epilogue_seq))
        {
          auto_sbitmap blocks (last_basic_block_for_fn (cfun));
          bitmap_clear (blocks);

          gcc_assert (BLOCK_FOR_INSN (epilogue_seq)->index
