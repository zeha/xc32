...
#ifdef _BUILD_MCHP_
/* MERGE-NOTES: this was ported from MIPS - was it from that original
   gcc version or something we added? added guard for now */
...
/* MERGE-NOTES: do_tlink_continue breaks broken configuration stuff in libgcc.
   a failed link due to a *$&# preprocessor option in the startfile_cxx_spec is
   causing us to configure libgcc in a certain way, and this
   failure to fail breaks things even worse. */
#if defined(_BUILD_MCHP_) && 0 /*&& defined(TARGET_MCHP_PIC32MX) */
