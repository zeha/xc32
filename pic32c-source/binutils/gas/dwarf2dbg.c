...
process_entries (segT seg, struct line_entry *e)
{
  ...
  do
      ...
      if (e->loc.discriminator != 0)
	{
      ...
      if (0)
        {
          /* Avoid "defined but never used" warning. */
          out_leb128 (0);
        }
#endif
	}
...
#if defined(TARGET_IS_PIC32C) || defined(TARGET_IS_PIC32MX)
      /* MERGE-TODO: str_seg is added to the code this replaces,
         so it's tentatively added here. It is not clear how
         to add the SEC_STRINGS | SEC_MERGE flags on top
         of the 'info' attribute... */
      PIC32_SET_INFO_ATTR(info_seg);
      PIC32_SET_INFO_ATTR(abbrev_seg);
      PIC32_SET_INFO_ATTR(aranges_seg);
#if defined(TARGET_IS_PIC32C)
      PIC32_SET_INFO_ATTR(str_seg);
#endif
#else
