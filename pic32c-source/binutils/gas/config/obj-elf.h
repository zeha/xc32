...
/* PIC32: kind of ugly, but we need to be able to have this use an existing
   section rather than forcing a new one to be created. This is because
   we need to create a section, process the attributes, and then
   pass those attributes to have obj_elf_change_section make sure all 
   flags are correct (see XC32E-683).
   We do this by supplying an optional section parameter, defaulting to NULL.
*/
#ifdef _BUILD_MCHP_
extern void obj_elf_change_section
  (const char *, unsigned int, unsigned int, bfd_vma, int, const char *,
   int, int, segT);
#define obj_elf_change_section(a,b,c,d,e,f,g,h) \
  (obj_elf_change_section)(a,b,c,d,e,f,g,h,NULL)
#define obj_elf_change_section_nocreate(a,b,c,d,e,f,g,h,sec) \
  (obj_elf_change_section)(a,b,c,d,e,f,g,h,sec)
#else
extern void obj_elf_change_section
  (const char *, unsigned int, unsigned int, bfd_vma, int, const char *,
   int, int);
