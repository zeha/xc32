...
static void pic32_attribute (int);
static int pic32_align_power (offsetT);
static bfd_vma pic32_address (bfd_vma);
static bfd_vma pic32_element_size (bfd_vma);
#if 0
/* TODO not added yet */
/* MERGE-TODO: another PIC32M thing - correct guards and add */
static void pic32_memory (int);
#endif
static void s_arm_change_section (int);
...
#ifdef TARGET_IS_PIC32C 
  { "section", s_arm_change_section, 0 },
  { "pushsection", s_arm_change_section, 1},
#endif
...
do_arit (void)
{
  ...
  if (pic32_gen_stack_usage == STACK_USAGE_ENABLED)
    {
      /* a bit of black magic to find what kind of insn this is... :-| */
      const int opcode = (inst.instruction >> DATA_OP_SHIFT) & 0xf;

      if (opcode == OPCODE_ADD || opcode == OPCODE_SUB)
        {
          const int Rd = inst.operands[0].reg;
          const bfd_boolean is_add = opcode == OPCODE_ADD;
          const int amount = inst.reloc.exp.X_add_number;
                  ...
                  if (Rd == REG_SP)
                    {
                      ...
                      pic32_su_alter_stack_depth (is_add ? -amount : amount);
                    }
                  /* add/sub fp, sp, #imm */
                  else if (Rd == REG_FP)
                    {
                      pic32_su_debug_msg ("\t%s fp, sp, #%d\n",
                                          (is_add ? "add" : "sub"), amount);
                      pic32_su_copy_stack_depth (/*back*/FALSE,
                                              (is_add ? -amount : amount));
                    }
                }
              else if (Rd == REG_SP && inst.operands[2].isreg)
                {
                  /* an add/sub sp, sp, rX may be a dynamic stack allocation */
                  pic32_su_debug_msg ("\t%s sp, sp, r%d\n",
                                      (is_add ? "add" : "sub"),
                                      inst.operands[2].reg);
                  pic32_su_alter_stack_depth_reg (/*decrease*/is_add,
                                               inst.operands[2].reg);
                }
            }
          else if (Rd == REG_SP
                   && inst.operands[1].reg == REG_FP
                   && !inst.operands[2].isreg
                   && inst.reloc.type == BFD_RELOC_ARM_IMMEDIATE)
            {
              /* add/sub sp, fp, #imm */
              pic32_su_debug_msg ("\t%s sp, fp, #%d\n",
                                  (is_add ? "add" : "sub"), amount);
              pic32_su_copy_stack_depth (/*back*/TRUE,
                                      (is_add ? -amount : amount));
            }
        }
    }
...
encode_ldmstm(int from_push_pop_mnem)
{
  ...
  if (pic32_gen_stack_usage == STACK_USAGE_ENABLED
    ...
    {
      const bfd_boolean is_push = !(inst.instruction & LOAD_BIT);

      /* the SP value is increased / decreased by this amount */
      const int amount = __builtin_popcount (range) << 2;

      pic32_su_debug_msg ("\t%s %u reg(s)\n",
                          (is_push ? "push" : "pop"), amount >> 2);
      pic32_su_alter_stack_depth (is_push ? amount : -amount);
    }
#endif
  ...
  if (pic32_gen_stack_usage == STACK_USAGE_ENABLED)
    {
      ...
      /* generic PC-rel load (e.g. ldr rX, .LLL) */
      if (is_load
          && inst.operands[1].isreg
          && inst.operands[1].reg == REG_PC
          && inst.operands[1].preind
          && inst.reloc.pc_rel
          && inst.reloc.exp.X_op == O_symbol)
        {
          pic32_su_handle_ldr (inst.operands[0].reg,
                               inst.reloc.exp.X_add_symbol,
                               inst.reloc.exp.X_add_number,
                               inst.reloc.type);
        }
      /* pushing one reg to the stack (w/ or w/o stk align) */
      else if (!is_load
          && inst.operands[1].isreg
          && inst.operands[1].reg == REG_SP
          && inst.operands[1].preind
          && inst.operands[1].writeback
          && !inst.operands[2].isreg
          && inst.reloc.exp.X_op == O_constant)
        {
          pic32_su_debug_msg ("\tstr r%d, [sp, #%d]!\n",
                              inst.operands[0].reg, inst.reloc.exp.X_add_number);
          pic32_su_alter_stack_depth (-inst.reloc.exp.X_add_number);
        }
      /* popping one reg from the stack (w/ or w/o stk align) */
      else if (is_load
          && inst.operands[1].isreg
          && inst.operands[1].reg == REG_SP
          && !inst.operands[1].preind
          ...
          && inst.reloc.exp.X_op == O_constant)
        {
          pic32_su_debug_msg ("\tldr r%d, [sp], #%d\n",
                              inst.operands[0].reg, inst.reloc.exp.X_add_number);
          pic32_su_alter_stack_depth (-inst.reloc.exp.X_add_number);
        }
    }
...
do_mov (void)
{
  ...
  if (pic32_gen_stack_usage == STACK_USAGE_ENABLED
      ...
      && inst.reloc.type == BFD_RELOC_ARM_IMMEDIATE)
    {
      pic32_su_handle_mov (inst.operands[0].reg, inst.reloc.exp.X_add_number);
    }
#endif
  inst.instruction |= inst.operands[0].reg << 12;
  encode_arm_shifter_operand (1);
...
do_t_add_sub_w (void)
{
  ...
  if (pic32_gen_stack_usage == STACK_USAGE_ENABLED
    ...
    {
      /* addw/subw sp, sp, #imm */
      const bfd_boolean is_add = !(inst.instruction & 0x00f00000);
      const int amount = inst.reloc.exp.X_add_number;

      pic32_su_debug_msg ("\t%s sp, sp, #%d\n",
                          (is_add ? "addw" : "subw"), amount);
      pic32_su_alter_stack_depth (is_add ? -amount : amount);
    }
#endif
...
do_t_add_sub (void)
{
  ...
  if (pic32_gen_stack_usage == STACK_USAGE_ENABLED)
    {
      const char *mnem = inst.instruction == T_MNEM_add  ? "add"  :
                         inst.instruction == T_MNEM_adds ? "adds" :
                         inst.instruction == T_MNEM_sub  ? "sub"  : "subs";

      const bfd_boolean is_add = inst.instruction == T_MNEM_add
      ...
      if (Rs == REG_SP)
        {
          if (!inst.operands[2].isreg)
          ...
          else if (Rd == REG_SP) /* inst.operands[2].isreg */
            {
              /* an add/sub sp, sp, rX may be a dynamic stack allocation */
              pic32_su_debug_msg ("\t%s sp, sp, r%d\n",
                                  mnem, inst.operands[2].reg);
              pic32_su_alter_stack_depth_reg (/*decrease*/is_add,
                                              inst.operands[2].reg);
            }
        }
      else if (Rs == Rd && Rs == REG_FP)
        {
          if (!inst.operands[2].isreg)
            {
              /* add/sub fp, fp, #imm */
              pic32_su_debug_msg ("\t%s fp, fp, #%d\n", mnem, amount);
              pic32_su_alter_saved_stack_depth (is_add ? -amount : amount);
            }
          else
            {
              /* add/sub fp, fp, rX */
              pic32_su_debug_msg ("\t%s fp, fp, r%d\n",
                                  mnem, inst.operands[2].reg);
              pic32_su_alter_saved_stack_depth_reg (/*decrease*/is_add,
                                                    inst.operands[2].reg);
            }
        }
    }
...
do_t_ldmstm (void)
{
  ...
  if (pic32_gen_stack_usage == STACK_USAGE_ENABLED
	  ...
	  && (inst.instruction == T_MNEM_ldmia
        || inst.instruction == 0xe9000000 /* there's no T_MNEM_stmdb */))
    {
      const bfd_boolean is_push = inst.instruction != T_MNEM_ldmia;

      /* the SP value is increased / decreased by this amount */
      const int amount = __builtin_popcount (inst.operands[1].imm) << 2;
  ...
  if (pic32_gen_stack_usage == STACK_USAGE_ENABLED)
    {
      ...
      /* STR can be used to push a reg on the stack (w/ or w/o stk align) */
      else if (opcode == T_MNEM_str
          && inst.operands[1].isreg
          && inst.operands[1].reg == REG_SP
          && inst.operands[1].preind
          && inst.operands[1].writeback
      ...
      /* LDR can be used to pop a word from the stack (w/ or w/o stk align) */
      else if (opcode == T_MNEM_ldr
          && inst.operands[1].isreg
          && inst.operands[1].reg == REG_SP
          && !inst.operands[1].preind
          && inst.operands[1].writeback
...
#ifdef TARGET_IS_PIC32C
  if (pic32_gen_stack_usage == STACK_USAGE_ENABLED
      && (inst.instruction == T_MNEM_mov || inst.instruction == T_MNEM_movs))
    {
      if (!inst.operands[1].isreg
          && inst.reloc.type == BFD_RELOC_ARM_IMMEDIATE)
        {
          pic32_su_handle_mov (Rn, inst.reloc.exp.X_add_number);
        }
      else if (Rn == REG_SP
               && inst.operands[1].isreg)
        {
          /* recognize the move back from the frame pointer */
          if (Rm == REG_FP)
            {
              pic32_su_debug_msg ("\tmov sp, fp\n");
              pic32_su_copy_stack_depth (/*back*/TRUE, 0);
            }
          else
            {
              /* a move to SP from another register means that there's a unknown,
               * variable adjustment */
              pic32_su_debug_msg ("\tmov sp, r%d\n", Rm);
...
do_t_push_pop (void)
{
  ...
  if (pic32_gen_stack_usage == STACK_USAGE_ENABLED)
    {
      const bfd_boolean is_push = inst.instruction == T_MNEM_push;
      /* the SP value is increased / decreased by this amount */
      const int amount = __builtin_popcount (mask) << 2;
...
#ifdef TARGET_IS_PIC32C
  /* 'observe' the left shifts with imm values when calculating stack usage */
  if (pic32_gen_stack_usage == STACK_USAGE_ENABLED
      && (inst.instruction == T_MNEM_lsls || inst.instruction == T_MNEM_lsl)
      && inst.operands[1].reg == inst.operands[0].reg
      && !inst.operands[2].isreg)
    {
      pic32_su_handle_lsl (inst.operands[0].reg,
                             inst.reloc.exp.X_add_number);
    }
#endif
...
do_vfp_nsyn_ldm_stm (int is_dbmode)
{
  ...
  if (pic32_gen_stack_usage == STACK_USAGE_ENABLED
      ...
      && inst.operands[0].writeback)
    {
      const int amount = inst.operands[1].imm * 4; /* how many fp regs */
                                                   /* 4 bytes each */
      pic32_su_debug_msg ("\t%s.32 %u reg(s)\n",
                          (is_ldm ? "vldm" : "vstm"), inst.operands[1].imm);
      pic32_su_alter_stack_depth (is_ldm ? -amount : amount);
    }
#endif
  if (is_ldm)
    {
  ...
  if (pic32_gen_stack_usage == STACK_USAGE_ENABLED)
    {
      const int amount = inst.operands[1].imm /* how many fp regs */
        << (2 + !inst.operands[1].issingle);  /* 4/8 bytes each */

      pic32_su_debug_msg ("\tvpush.%s %u reg(s)\n", (
                          inst.operands[1].issingle ? "32" : "64"),
                          inst.operands[1].imm);
  ...
  if (pic32_gen_stack_usage == STACK_USAGE_ENABLED)
    {
      ...
      pic32_su_debug_msg ("\tvpop.%s %u reg(s)\n", (
                          inst.operands[1].issingle ? "32" : "64"),
                          inst.operands[1].imm);
      pic32_su_alter_stack_depth (-amount);
    }
...
do_neon_ldm_stm (void)
{
  ...
  if (pic32_gen_stack_usage == STACK_USAGE_ENABLED
      && inst.operands[0].reg == REG_SP)
    {
      const int is_ldm = (inst.instruction & (1 << 20)) != 0;
      const int amount = inst.operands[1].imm * 8; /* how many fp regs */
                                                   /* 8 bytes each */
      pic32_su_debug_msg ("\t%s.64 %u reg(s)\n",
                          (is_ldm ? "vldm" : "vstm"), inst.operands[1].imm);
      pic32_su_alter_stack_depth (is_ldm ? -amount : amount);
    }
#endif
...
#ifdef TARGET_IS_PIC32C
  /* Bump no. of current instruction */
  ++pic32_insn_no;
#endif
...
#ifdef TARGET_IS_PIC32C
  if (pic32_gen_stack_usage != STACK_USAGE_DISABLED)
    {
      flagword flags = symbol_get_bfdsym (sym)->flags;
      /* keep the last seen global symbol for the situation of encountering
       * a stack-using instruction w/o previously seeing a function sym */
      if (flags & BSF_GLOBAL)
        last_global_sym = sym;
      if ((ARM_GET_FLAG (sym) & THUMB_FLAG_FUNC) || (flags & BSF_FUNCTION))
        pic32_su_start_function (S_GET_NAME (sym));
    }
#endif
...
arm_fix_adjustable (fixS * fixP)
{
  ...
  if (	 fixP->fx_r_type == BFD_RELOC_VTABLE_INHERIT
    ...
    /* MERGE-TODO: another MIPS-only thing to maybe turn on. */
#ifdef TARGET_IS_PIC32C
...
struct arm_option_table arm_opts[] =
{
  ...
  {"mlittle-endian", N_("assemble for little-endian"), &target_big_endian, 0,
   NULL},
#ifdef TARGET_IS_PIC32C
  {"mchp-stack-usage", N_("generate stack usage info"), &pic32_gen_stack_usage,
   STACK_USAGE_ENABLED, NULL},
#endif
...
#ifdef TARGET_IS_PIC32C
  if (pic32_gen_stack_usage != STACK_USAGE_DISABLED)
    pic32_su_write_stack_usage_sections ();
#endif
...
/*
 Processes the attribute list of a bfd section, updating the
 attribute map and the section fields and flags
 */
static void
...
#include "pic32c-attributes.h"
    ...
    if (flag_debug)
        printf ("    pic32_attribute::map = %x flags = %x\n", attribute_map, sec->flags);
...
/*
 Processes the "new-style" .section directive described below:
 
 The standard binutils .section directive is of the form
 .section name [, "flags"[, @type[,flag_specific_arguments]]]
 
 For PIC32C, we have added a new section directive:
 .section name [, attr1[,...,attrn]]
 This new .section directive takes a list of attributes rather than
 section flags.
 */
#ifdef TARGET_IS_PIC32C
...
static void
s_arm_change_section (int push)
{
    bfd_boolean has_flags = FALSE;
    char *save, *stmp, ctmp;
    int result;

    /* this code tries to look ahead for things that aren't attributes,
       but gets confused by (legal) quoted section names. we try to at least use the
       standard function to move past the name before checking anything else. */
    save = input_line_pointer;
    ctmp = get_symbol_name (&stmp);
    (void) restore_line_pointer (ctmp);
    ...
    if (has_flags) {
        ...
        /* need these to ensure obj_elf_change_section handles the 
           stack correctly. */
        segT saved_previous_section = previous_section;
        segT saved_now_seg = now_seg;
        int saved_previous_subsection = previous_subsection;
        int saved_now_subseg = now_subseg;
        ...
        if ((c == '*') && (strlen(section_name) == 0))
        {
	        ...
	        cc = get_symbol_name (&p);

            if (strlen(p) > 0) {  /* if a name is provided, use it */
                name = xmalloc (input_line_pointer - p + 1);
                strcpy (name, p);
            } else {              /* else invent a name */
                /* MERGE_NOTES: 'filename' was uninitialized and used as a prefix. it's gone now. */
                as_where( &lineno);
                len = strlen(s) + 5;
                name = xmalloc (len + 1);
                snprintf(name, len, "%s%d", s, ++auto_name_cnt);
            }
            (void) restore_line_pointer (cc);
        }
        else if (strlen(section_name) == 0)
        {
            as_bad (_("invalid section name (none)\n"));
            name = xmalloc (strlen("(none)") + 1);
            sprintf (name, "(none)");
            (void) restore_line_pointer (c);
        }
        else
        {
            ...
            /* if stack usage info is found in the source, don't attempt to interact
             * with it, just disable the stack usage generation from the assembler */
            if (pic32_gen_stack_usage != STACK_USAGE_DISABLED
                && !(strcmp (name, SU_SECTION) && strcmp (name, SU_SECTION_HDR)))
              {
                if (flag_debug)
                  printf ("stack usage section(s) found, disable "
                          "-mchp-stack-usage automatic generation for this file\n");
                pic32_su_disable_gen_stack_usage (/*whole_file*/TRUE);
              }
        ...
        /* since we're adding the section ahead of elf_arm_change_section,
           we need to take the responsibility of associating the name
           with the BFD section. */
        ...
        if (sym_val) {
            ...
            if (sym_val) {
              symbol_name = xmalloc (strlen(sec->name) + strlen(EXT_ATTR_PREFIX) + 1);
              (void) sprintf(symbol_name, "%s%s", EXT_ATTR_PREFIX, sec->name);
              if (!symbol_find(symbol_name)) {
                symbolp = symbol_new (symbol_name, absolute_section, sym_val,
                                      &zero_address_frag);
                symbol_table_insert (symbolp);
                symbol_mark_resolved (symbolp);
              }
	...
	/* BSS sections are SHF_ALLOC with SHT_NOBITS. elf_section_type doesn't
	   know about the BSS attribute, nor persist, so we clean it up here. */
	if (PIC32_IS_BSS_ATTR (sec) || PIC32_IS_BSS_ATTR_WITH_MEMORY_ATTR (sec)
	    || PIC32_IS_PERSIST_ATTR (sec))
	  elf_section_type (sec) = SHT_NOBITS;

	/* MERGE-NOTES: ELF flags are not updated correctly by our replacement
	   hook, causing unexpected flags, e.g. write for init_array and
	   fini_array. The following maps our BFD flags back to ELF flags (yeah,
	   yeah...) and hands off to the standard elf section code, having
	   processsed our magic. */
	int flags = sec->flags;
	int type = elf_section_type (sec);

	bfd_vma attr = ((flags & SEC_READONLY) ? 0 : SHF_WRITE)
	...
	/* make obj_elf_change_section see the world as it was before we called
	   subseg_new. */
	previous_section = saved_previous_section;
	previous_subsection = saved_previous_subsection;
	now_seg = saved_now_seg;
	now_subseg = saved_now_subseg;
      obj_elf_change_section_nocreate (name, type, 0, attr, 0, NULL, FALSE, push, sec);
...
/* TODO not added yet */
static void
pic32_memory (int unused ATTRIBUTE_UNUSED)
{
