...
/* MERGE-TODO: wow. this was guarded with TARGET_IS_PIC32MX, with
   the following block for -plugin guarded only by ENABLE_PLUGINS.
   See if these issues even occur outside of MIPS (and why we need -processor
...
#ifdef ENABLE_PLUGINS
    /* The -plugin and -plugin-opt options are ambigious because of the
     -p (processor) option.  They will be considered as processors
     called "lugin" or "lugin-opt".  To avoid the ambiguity, convert
     the option to use two hyphens and so that it is recognized as a
     long option. */
    for (i = 1; i < argc; ++i)
        if (strncmp (argv[i], "-plugin", 7) == 0)
        {
            size_t len = strlen (argv[i]) + 2;
            char *new_arg = (char *) xmalloc (len);
            snprintf (new_arg, len, "-%s", argv[i]);
            argv[i] = new_arg;
        }
#endif /* ENABLE_PLUGINS */
