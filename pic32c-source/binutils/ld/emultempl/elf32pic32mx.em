...
static void pic32_after_open (void);
         ...
         if (undefs) {
           ...
           if (!full) {
             ...
             for (l = smartio_fn_list; l; l=l->next) {
               ...
               ((struct elf_link_hash_entry *)l->h)->type =
                 ((struct elf_link_hash_entry *)full)->type;
               su_note_smartio_replacement (l->h->root.string, buffer_map_to);
             }
  ...
  if (pic32_data_init)
    ...
    if ((shared_data_sections != NULL) && (shared_data_sections->sec != NULL))
      ...
      if (sec)
        {
          sec->size = total_data;
          sec->flags |= (SEC_HAS_CONTENTS | SEC_IN_MEMORY | SEC_LOAD | SEC_CODE | SEC_KEEP | SEC_READONLY);
    ...
    bfd_vma size, struct pic32_section* s)
{
    ...
    for (b = prev; b != NULL; b = b->next)
    {
        ...
        if (pic32_debug)
            printf("LG - free block 0x%lx - 0x%lx\n", b->addr, b->size);
        ...
        {
            if (pic32_debug)
                printf("LG - return free block 0x%lx - 0x%lx & offset 0x%lx\n", b->addr, b->size, offset);
            return b;
        }
    }
      ...
      if (sec)
        ...
        if (init_shared_template != NULL)
        {
            ...
            if (sec)
            {
                ...
                if (pic32_debug)
                    printf("Creating __shared_dinit_addr = %lx\n", shared_dinit_addr);
