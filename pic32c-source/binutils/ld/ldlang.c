...
/* MERGE-NOTES: this used to unconditionally ensure PIC32 and TARGET_IS_PIC32C
...
/* MERGE-TODO: like the above this is probably needed for mips too. */
#if !defined(TARGET_IS_PIC32C) && !defined(TARGET_IS_PIC32MX)
static
#endif
...
/*MERGE NOTES: to be merged with pic32c version */
#ifdef TARGET_IS_PIC32MX
/* functions defined in bfd/pic32-attributes.c */
extern unsigned int pic32_attribute_map
  (asection *);
	   ...
	   bfd_boolean add_to_list)
{
  ...
  /* MERGE-NOTES: maybe keep this mips-only? */
#if defined(TARGET_IS_PIC32C) || defined(TARGET_IS_PIC32MX)
		  ...
		  lang_output_section_statement_type *output)
{
  ...
  */
  /* MERGE-NOTES - as you can see, most flags are common and some are 
     not. we'll have to decide whether to unify the flags or... */
#if defined(TARGET_IS_PIC32MX)
  ...
  */
  ...
  if (PIC32_IS_ABSOLUTE_ATTR (section))
	...
	einfo (_ ("%F%P: Link Error: address for input section \'%s\'"
		  ...
		  " \'%s\' (%lx) \n"),
	       section->name, section->vma, section->output_section->name,
...
#if defined(TARGET_IS_PIC32MX)
  /* MERGE-TODO: I think several 'why' todos are all related to
     optional libraries or files... */
  if (entry->the_bfd == 0)
    return TRUE;
#endif
...
#if defined(TARGET_IS_PIC32MX) /* lghica co-resident - to be changed */
		 ...
		 bfd_vma rbase)
{
  if ((region->current < region->origin
	...
	{
#if !defined(TARGET_IS_PIC32C) && !defined(TARGET_IS_PIC32MX)
      /* MERGE-NOTES: this is a pretty egregious thing to outright disable -
         I assume it is for absolute sections which are probably not in any
         region in the script? */
...
#if defined(TARGET_IS_PIC32C) || defined(TARGET_IS_PIC32MX)
		    /* MERGE-NOTES: I assume this is because we completely
		       bypass the default orphan handling - seems unneccessary.
		       where should they be mapped? */
		    if ((strcmp (os->bfd_section->name, ".debug_ranges") == 0)
			|| (strcmp (os->bfd_section->name, ".debug_pubtypes")
			    == 0)
			|| (strcmp (os->bfd_section->name, ".gnu.attributes")
			    == 0))
		      {
			newdot = 0;
			if (issue_warning)
			  {
			    einfo (
			      _ ("%P: warning: Sections: .debug_ranges, "
...
#if defined(TARGET_IS_PIC32C) || defined(TARGET_IS_PIC32MX)
              /* MERGE-TODO: cf to previous gripes about orphan handling..
                 also be wary if MIPS has different but similar needs. */
              if ((strcmp(os->bfd_section->name,".debug_ranges") == 0) ||
  ...
  for (s = link_info.output_bfd->sections; s != NULL; s = s->next)
    {
      ...
      sprintf (symbol, ".startof.%s", secname);
         ...
         */
      /* MERGE-TODO: this code has been refactored. I'm inserting this
         at what I think is the right spot. check it. */
      (void) strtok(symbol, "%"); /* insert null at the first occurrence of % */
#endif
      lang_define_start_stop (symbol, s);

      memcpy (symbol + 1, ".size", 5);
#if defined(TARGET_IS_PIC32C) || defined(TARGET_IS_PIC32MX)
      /* MERGE-TODO: this hopefully has the same effect - the old code used
         a fresh buffer with ".sizeof.%s" before someone 'optimized' it. */
      (void) strtok(symbol + 1, "%");
#endif
      lang_define_start_stop (symbol + 1, s);
      free (symbol);
    }
...
static void
#if defined(TARGET_IS_PIC32MX)
ldlang_place_orphan (lang_input_statement_type *file, asection *s)
#else
...
#ifdef TARGET_IS_PIC32MX
      os = ldemul_place_orphan (file, s, name, constraint);
#else
  ...
  if (link_info.lto_plugin_active)
      ...
      if (added.head != NULL)
	{
	  ...
	  while (files.head != NULL)
        ...
        The "while (files.head != NULL)" loop ahead updates the file_chain list
        (by doing some weird copy operations so that "All elements from an
        archive can be added at once" ) but fails to update the file_chain.tail
        field. This file_chain.tail field is very important because it used by
        the lang_statement_append to insert new files into the file_chain list.

        MIPS arch calls lang_statement_append several times after the current
        routine return

        bfd_pic32_add_bfd_to_link (init_bfd, init_bfd->filename);
        bfd_pic32_add_bfd_to_link (stack_bfd, stack_bfd->filename);

        and causes the file_chain list to be inconsistent (init_bfd, stack_bfd,
        etc will be inserted in the middle of the file_chain list)

       */
          {
            lang_input_statement_type *last_elem;
            LANG_FOR_EACH_INPUT_STATEMENT (f)
            {
              if (f->next == NULL)
                last_elem = f;
            }
            file_chain.tail = &(last_elem->next);
          }
        }
    }
#endif /* ENABLE_PLUGINS */
  ...
  if (pic32_stack_usage)
    {
      ...
      if (mchpStackUsageValidLicense ())
        ...
        {
          einfo (_("%Q: warning: Option -mchp-stack-usage ignored due "
                   "to an invalid XC32 license.\n"));
          einfo (_("%Q: note: Disable the option or visit "
                   "http://www.microchip.com/MPLABXCcompilers "
                   "to purchase a new MPLAB XC compiler license.\n"));
          pic32_stack_usage = FALSE;
        }
#endif
    }
#endif
...
#if defined(TARGET_IS_PIC32C) || defined(TARGET_IS_PIC32MX)
#if 0 ///\ tmp removed serialmem support - lghica
...
#endif
    //  fixme:  what is this and where is it declared and used?  commented out for now
...
#ifdef TARGET_IS_PIC32C
      /* PIC32C: again deal with garbage region_lookup artifacts */
      /* MERGE-TODO: this shouldn't happen anymore, change to assert */
