/* Best-fit allocator (BFA) for pic32m.
...
/* Stuff from other files that are not in headers. */
...
/* MERGE-NOTES: adding prefixes to attribute-related declarations to avoid
   warnings related to having common names like 'align' in the namespace.
   NB: if the 'a' macro argument can be a macro itself, the usage of ##
   below will need to be fixed to ensure expansion occurs before catenation. */

/* Create an id for each section attribute */
#define ATTR(a,b,c)  attr_id_##a,
 enum {
#include "pic32m-attributes.h"
 } ;

/* Create a bit mask for each section attribute */
#undef ATTR
#define ATTR(a,b,c)  attr_mask_##a = (1u<<attr_id_##a),
 enum {
#include "pic32m-attributes.h"
 } ;

/* Create a bit mask to match any attribute */
#define attr_mask_all_attr 0xFFFFFFFF
...
static void build_free_block_list(struct memory_region_struct *, unsigned int);
static int locate_sections(unsigned int, unsigned int,
			   struct memory_region_struct *);
static int locate_coherent_sections (unsigned int,
				     struct memory_region_struct *);
static void update_group_section_info(bfd_vma,
				      struct pic32_section *,
				      struct memory_region_struct *);
static struct pic32_memory *select_free_block(struct pic32_section *,
					      unsigned int);
static void remove_free_block(struct pic32_memory *);
...
void swap_sections_within_group (struct pic32_section *a,
				 struct pic32_section *b);
...
#define PREFIX "/tmp"
    ...
    PIC32_DEBUG("\n    Error: Could not allocate section %s", s->sec->name);
}
 ...
 *
 */
static void update_shared_dinit_addr(void)
{
    ...
    {
        //PIC32_DEBUG("LG --- .shared.dinit LMA 0x%x", shared_dinit_sec->lma);
    ...
    {
        //PIC32_DEBUG("LG --- shared_dinit_addr - 0x%x", h_sym->u.def.value);
...
#define ERR_STR " Link Error: Could not allocate "
  ...
  last_os = (lang_output_section_statement_type **) stat_ptr->tail;
    ...
    for (os = (lang_statement_union_type *) *last_os;
	  ...
	  if (next->output_section_statement.children.head) {
          ...
          if (match && !config.unique_orphan_sections &&
            ...
            DEBUG({
		LOG("  Merging output sections %s and %s\n",
		    sec->name, next_sec->name);
		LOG("    %s: addr = %lx, len = %lx\n",
...
allocate_program_memory() {
  ...
  unsigned int mask = attr_mask_code;
  int result = 0;
...
#if 0
  DEBUG(pic32_print_section_list(unassigned_sections, "unassigned"));
#endif

  reset_locate_options();
  result |= locate_sections(attr_mask_address, 0, region);   /* most restrictive  */
  result |= locate_sections(attr_mask_all_attr, 0, region);  /* least restrictive */
...
allocate_serial_memory() {
  struct memory_region_struct *region;
  unsigned int mask = attr_mask_serial_mem;
  int result = 0;
...
static int allocate_bss(struct memory_region_struct * const region)
{
    ...
    PIC32_DEBUG("\nBuilding bss allocation list attribute mask = %x", attr_mask_bss);
...
allocate_data_memory() {
  ...
  /* XC32-2376 we need to grab all coherent sections here so they can be
   * grouped */
  unsigned int mask = attr_mask_data | attr_mask_persist | attr_mask_coherent
  ...
  else
  {
    ...
    if (region0 && region1)
    {
      PIC32_DEBUG("region0->length = %lx, region1->length = %lx",
  ...
  reset_locate_options();
  result |= locate_sections(attr_mask_ramfunc, 0, region);    /* most restrictive  */
  ...
  result |= locate_sections(attr_mask_address, 0, region);
    
  coherent_section_count = 0;
  result |= locate_coherent_sections(attr_mask_data, region);  // allocate coherent data sections
    
  coherent_section_count = 0;
  result |= locate_coherent_sections(attr_mask_bss, region);	 // allocate coherent  sections
    
  coherent_section_count = 0;
  result |= locate_coherent_sections(attr_mask_persist, region);	 // allocate coherent persist sections
  ...
  result |= locate_sections(attr_mask_heap, 0, region);

  /* user-defined stack */
  if (ramfunc_begin != 0) {
    set_locate_options(EXCLUDE_HIGH_ADDR, ramfunc_begin);
  }
  result |= locate_sections(attr_mask_stack, 0, region);
#else
 ...
 */
 ...
 locate_group_section(struct pic32_section *s,
  ...
  /* DEBUG */
  PIC32_DEBUG("  group section \"%s\", total size = %lx",
	      s->sec->name, len);
  /* 
    XC32-1581 - remove the upper 32b of an address that is knwon at build time:

      when the linker reads this address from the object file, a sign extension
      is performed => the upper 32b of the bfd_vma might be filled with 0xFFFFFFFF
  */
  ...
  PIC32_DEBUG("    removing group from allocation list");
 ...
 locate_single_section(struct pic32_section *s,
  ...
  if (PIC32_IS_ABSOLUTE_ATTR (s->sec))
    {
     ...
     if (s->sec)
       PIC32_DEBUG("  %s with len %ld & addr %lx", s->sec->name, len, addr);
  ...
  if (len == 0)
    ...
    if (b) {
	  ...
	  PIC32_DEBUG("LG --- SECTION in shared_data_memory %s (0x%lx - 0x%lx)",
  ...
  PIC32_DEBUG("    removing from allocation list");
  pic32_remove_from_section_list(alloc_section_list,s);
...
locate_sections(unsigned int mask, unsigned int block,
  ...
  DEBUG({
      ...
      LOG("  locate_options = %x, exclude_addr = %lx\n",
  ...
  for (s = alloc_section_list; s != NULL; s = next) {
    ...
    if (s->sec && (s->attributes & mask) &&
      ...
      if (PIC32_IS_RAMFUNC_ATTR(s->sec)) {
	      ...
	      PIC32_DEBUG("  _ramfunc_begin = %x", (unsigned int)ramfunc_begin);
	      ...
	      PIC32_DEBUG("  _bmxdkpba_address = %x", (unsigned int)(ramfunc_begin - region->origin));
            }
        }
      else {
...
/* lghica co-resident */
  ...
  if (PIC32_IS_ABSOLUTE_ATTR(s->sec))
  {
          bfd_vma addr = s->sec->lma & 0x00000000FFFFFFFF;
          b = pic32_static_assign_memory(free_blocks, len, addr);
    if (!b) {
  ...
  for (b = free_blocks; b != NULL; b = b->next) {
    ...
    PIC32_DEBUG("    consider block at %lx, len = %lx", b->addr, b->size);
    ...
    PIC32_DEBUG("    option1 [advancing from %lx]", option1 + 2);
    while (1) {
      ...
      PIC32_DEBUG("    option1 aligned at %lx", option1);
	...
	PIC32_DEBUG("    approaching EXCLUDE boundary from %lx", option1);
        continue;
      }
    ...
    limit2  = b->addr;
      ...
      PIC32_DEBUG("    option2 [descending from %lx]", option2 - 2);
      while (1) {
	...
	PIC32_DEBUG("    option2 aligned at %lx", option2);
	  ...
	  PIC32_DEBUG("    approaching Near boundary from %lx", option2);
          continue;
        }
	  ...
	  PIC32_DEBUG("    approaching EXCLUDE boundary from %lx", option2);
          continue;
        }
	...
	LOG("    ");
	if (option1_valid)
	  LOG("option1 = %lx, ", option1);
	if (option2_valid)
	  LOG("option2 = %lx", option2);
	LOG("\n");
      });
...
update_section_info(bfd_vma alloc_addr,
  ...
  PIC32_DEBUG("    updating section info:"
	      "  vma = %lx, lma = %lx", s->sec->vma, s->sec->lma);
  ...
  PIC32_DEBUG("    creating output section statement 2\"%s\"", os->name);
  ...
  finish_section_info(s, os);
    ...
    {
            PIC32_DEBUG("LG Section %s set abs & linked", s->sec->name);
...
update_group_section_info(bfd_vma alloc_addr,
  ...
  for (s = g; s != NULL; s = next) {
    ...
    if (s->sec && (PIC32_IS_RAMFUNC_ATTR(s->sec))) {
      ...
      PIC32_DEBUG("    updating grouped section info:"
		  "  vma = %lx, lma = %lx", s->sec->vma, s->sec->lma);
    }
  }
...
create_remainder_blocks(struct pic32_memory *lst,
  ...
  PIC32_DEBUG("    creating remainder blocks: %lx, %lx",
	      b->offset, remainder);
 ...
 *
 * Note: so far we are not doing iterative allocation for mips,
 * so here we just initialize pic32_size and children to 0.  When we do that
 * we'll have to make this look more like the arm version.
 */
static void
...
build_alloc_section_list(unsigned int mask) {
  ...
  for (s = prev; s != NULL; s = next) {
    ...
    if (s->sec && (s->sec->flags & SEC_EXCLUDE))
      continue;
    /* XC32-2416 - the sections which should be in user defined memory 
    shouldn't be placed in the allocation list at this point */
    if ((s->attributes & mask) && !(has_user_defined_memory && PIC32_IS_MEMORY_ATTR(s->sec))) {
...
build_free_block_list(struct memory_region_struct *region,
  ...
  for (s = pic32_section_list; s != NULL; s = s->next) {
      ...
      if (s->sec)
        PIC32_DEBUG("    Checking (section \"%s\", lma = %lx, vma = %lx, len = %lx, attr = %x)",
    ...
    if ((s->sec) && ((s->sec->rawsize | s->sec->size) > 0) && ma) {
      PIC32_DEBUG("    (section \"%s\", addr = %lx, len = %lx, attr = %x)",
		  s->sec->name, ma,
		  s->sec->rawsize? s->sec->rawsize : s->sec->size,
		  s->attributes);
	...
	PIC32_DEBUG("  block %d, addr = %lx, len = %lx",
		    ++cnt, dot, len);
...
#define KSEG1_DATA_MEM_LIMIT 0xFFFFFFFFul;
  ...
  for (b = data_memory_free_blocks; b != NULL; b = next) {
    ...
    if ((b->addr < max_addr) && ((b->addr + b->size) > max_addr)) {
      ...
      PIC32_DEBUG("  free block at %lx crosses upper stack limit (%lx)",
		  b->addr, max_addr);
  ...
  DEBUG({
      LOG("  selecting block at %lx\n", big_block->addr);
      LOG("  heap base = %x, heap limit = %x\n", heap_base, heap_limit);
      LOG("  stack base = %x, stack limit = %x\n", stack_base, stack_limit);
    });
...
check_overlapping_memory_regions (asection **asec_list, unsigned int asec_count)
{
  ...
  /* MERGE-NOTES: FIXME? asec_count == 0 would mean an underflow here. */
...
static int 
validate_memory_region_list(void)
{
...
#if 0
      PIC32_DEBUG("s->file =%s, s->sec = %s ", (char *) s->file, (char *) s->sec);
#endif
...
//    temp_region.old_length = 0;
    ...
    result |= locate_sections(attr_mask_address, 0, &temp_region);   /* most restrictive  */
    result |= locate_sections(attr_mask_all_attr, 0, &temp_region);  /* least restrictive */
  }
...
update_coherent_group_section_info(bfd_vma alloc_addr,
	...
	if (PIC32_IS_COHERENT_ATTR (g->sec)
	    && ((g->attributes & mask)
		== (attr_mask_coherent | attr_mask_data)))
	  {
            strcpy(name, ".COHERENTDATA$");
          }
	else if (PIC32_IS_COHERENT_ATTR (g->sec)
		 && ((g->attributes & mask)
		     == (attr_mask_coherent | attr_mask_bss)))
	  {
            strcpy(name, ".COHERENTBSS$");
          }
	else if (PIC32_IS_COHERENT_ATTR (g->sec)
		 && ((g->attributes & mask)
		     == (attr_mask_coherent | attr_mask_persist)))
	  {
