...
#if defined(TARGET_IS_PIC32C) || defined(TARGET_IS_PIC32MX)
  /* XC32-1531: SHT_NOTE and 'info' are not equivalent. ensure
     'info' is set for things that aren't ALLOC, at least. */
  if (hdr->sh_type == SHT_NOTE && !(hdr->sh_flags & SHF_ALLOC))
...
#if 1 /* !defined(TARGET_IS_PIC32C) && !defined(TARGET_IS_PIC32MX) */
      if (!(hdr->p_flags & PF_W))
	    {
...
elf_fake_sections (bfd *abfd, asection *asect, void *fsarg)
{
...
#if 0 /* defined(TARGET_IS_PIC32C) || defined(TARGET_IS_PIC32MX) */
  ...
  if ((asect->flags & SEC_READONLY) == 0)
    this_hdr->sh_flags |= SHF_WRITE;
#endif
#if 0 /* defined(TARGET_IS_PIC32MX) */
...
#if 0 /* defined(TARGET_IS_PIC32MX) || defined(TARGET_IS_PIC32C) */
      if (PIC32_SECTION_IN_DATA_MEMORY(hdr))
#else
      if ((hdr->flags & SEC_READONLY) == 0)
#endif
					     ...
					     struct bfd_link_info *link_info)
{
  ...
  for (hdrpp = i_shdrpp + 1; hdrpp < end_hdrpp; hdrpp++)
	...
	{
	  if ((hdr->sh_size != 0)
#if defined(TARGET_IS_PIC32MX)   ///\coresident breaks the allocation rules from linker script when building
