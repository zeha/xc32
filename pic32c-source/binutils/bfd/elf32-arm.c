...
#if !defined(TARGET_IS_PIC32C) /* defined in elf32-pic32c.c */
...
/* MERGE_NOTES: lots of info parameter renames to avoid shadowing the 
   info attribute global symbol. reconsider those being globals.. */

static void elf32_arm_add_dynreloc (bfd *output_bfd, struct bfd_link_info *hinfo,
				    asection *sreloc, Elf_Internal_Rela *rel);

static void
arm_elf_fill_funcdesc(bfd *output_bfd,
		      struct bfd_link_info *hinfo,
		      int *funcdesc_offset,
		      int dynindx,
		      int offset,
		      bfd_vma addr,
		      ...
		      bfd_vma seg)
{
  if ((*funcdesc_offset & 1) == 0)
    {
      struct elf32_arm_link_hash_table *globals = elf32_arm_hash_table (hinfo);
      asection *sgot = globals->root.sgot;

      if (bfd_link_pic(hinfo))
	{
	  ...
	  elf32_arm_add_dynreloc (output_bfd, hinfo, srelgot, &outrel);
...
static bfd_boolean
elf32_arm_plt_needs_thumb_stub_p (struct bfd_link_info *hinfo,
				  struct arm_plt_info *arm_plt)
{
...
static bfd_boolean
create_got_section (bfd *dynobj, struct bfd_link_info *hinfo)
{
  struct elf32_arm_link_hash_table *htab;
  ...
  if (! _bfd_elf_create_got_section (dynobj, hinfo))
    return FALSE;
...
static bfd_boolean
create_ifunc_sections (struct bfd_link_info *hinfo)
{
  struct elf32_arm_link_hash_table *htab;
...
static bfd_boolean
elf32_arm_create_dynamic_sections (bfd *dynobj, struct bfd_link_info *hinfo)
{
  struct elf32_arm_link_hash_table *htab;
  ...
  if (!htab->root.sgot && !create_got_section (dynobj, hinfo))
    return FALSE;

  if (!_bfd_elf_create_dynamic_sections (dynobj, hinfo))
    return FALSE;

  if (htab->vxworks_p)
    {
      if (!elf_vxworks_create_dynamic_sections (dynobj, hinfo, &htab->srelplt2))
	return FALSE;
  ...
  if (!htab->root.splt
      || !htab->root.srelplt
      || !htab->root.sdynbss
      || (!bfd_link_pic (hinfo) && !htab->root.srelbss))
    abort ();
...
static void
elf32_arm_copy_indirect_symbol (struct bfd_link_info *hinfo,
				...
				struct elf_link_hash_entry *ind)
{
  ...
  _bfd_elf_link_hash_copy_indirect (hinfo, dir, ind);
}
...
arm_type_of_stub (struct bfd_link_info *hinfo,
		  asection *input_sec,
		  ...
		  const char *name)
{
  ...
  globals = elf32_arm_hash_table (hinfo);
  if (globals == NULL)
    return stub_type;
	      ...
	      && !use_plt))
	{
	  ...
	  if (branch_type == ST_BRANCH_TO_THUMB)
	      ...
	      if (!thumb_only)
		{
		  ...
		  stub_type = (bfd_link_pic (hinfo) | globals->pic_veneer)
		    /* PIC stubs.  */
		    ? ((globals->use_blx
			&& (r_type == R_ARM_THM_CALL))
		  ...
		  && !INTERWORK_FLAG (sym_sec->owner))
		{
	      ...
	      stub_type =
		(bfd_link_pic (hinfo) | globals->pic_veneer)
		/* PIC stubs.  */
		? (r_type == R_ARM_THM_TLS_CALL
		   /* TLS PIC stubs.  */
...
static unsigned
elf32_arm_tls_transition (struct bfd_link_info *hinfo, int r_type,
...
#define MAXRELOCS 3
  ...
  hinfo = (struct bfd_link_info *) in_arg;
  ...
  for (i = 0; i < nrelocs; i++)
	...
	points_to = stub_entry->target_section->output_section->vma
	  ...
	  (template_sequence[stub_reloc_idx[i]].r_type),
	   stub_bfd, hinfo->output_bfd, stub_sec, stub_sec->contents, &rel,
	   points_to, hinfo, stub_entry->target_section, "", STT_FUNC,
			       ...
			       struct bfd_link_info *hinfo)
{
  ...
  bfd_size_type amt;
  struct elf32_arm_link_hash_table *htab = elf32_arm_hash_table (hinfo);
  ...
  for (input_bfd = hinfo->input_bfds, bfd_count = 0, top_id = 0;
       input_bfd != NULL;
...
void
elf32_arm_next_input_section (struct bfd_link_info *hinfo,
			      asection *isec)
{
  struct elf32_arm_link_hash_table *htab = elf32_arm_hash_table (hinfo);
		  ...
		  if (found)
		    {
			 ...
			 function, only if there is glue or not.  */
		      entry = find_thumb_glue (hinfo, found->sym_name,
					       &error_message);
...
arm_list_new_cmse_stub (struct bfd_hash_entry *gen_entry, void *gen_info)
{
  ...
  /* Massage our args to the form they really have.  */
  stub_entry = (struct elf32_arm_stub_hash_entry *) gen_entry;
  hinfo = (struct bfd_link_info *) gen_info;

  if (hinfo->out_implib_bfd)
    return TRUE;
...
/* Set offset of each secure gateway veneers so that its address remain
   ...
   (hinfo->out_implib_bfd is NULL and *CMSE_STUB_CREATED is bigger than the
...
static bfd_boolean
set_cmse_veneer_addr_from_implib (struct bfd_link_info *hinfo,
				  struct elf32_arm_link_hash_table *htab,
				  int *cmse_stub_created)
{
  long symsize;
  ...
  if (!hinfo->out_implib_bfd && new_cmse_stubs_created != 0)
    {
      BFD_ASSERT (new_cmse_stubs_created > 0);
      _bfd_error_handler
	(_("new entry function(s) introduced but no output import library "
	   "specified:"));
      bfd_hash_traverse (&htab->stub_hash_table, arm_list_new_cmse_stub, hinfo);
    }
  ...
  while (1)
    {
      ...
      num_a8_fixes = 0;
      for (input_bfd = hinfo->input_bfds, bfd_indx = 0;
	   input_bfd != NULL;
	   input_bfd = input_bfd->link.next, bfd_indx++)
	{
		  ...
		  do
		    {
		      ...
		      stub_type = arm_type_of_stub (hinfo, section, irela,
						    st_type, &branch_type,
	  ...
	  if (htab->fix_cortex_a8)
	    {
	      ...
	      if (cortex_a8_erratum_scan (input_bfd, hinfo, &a8_fixes,
	  ...
	  && !set_cmse_veneer_addr_from_implib (hinfo, htab,
						&cmse_stub_created))
	ret = FALSE;
...
bfd_boolean
elf32_arm_build_stubs (struct bfd_link_info *hinfo)
{
  asection *stub_sec;
  struct bfd_hash_table *table;
  enum elf32_arm_stub_type stub_type;
  ...
  bfd_hash_traverse (table, arm_build_one_stub, hinfo);
  if (htab->fix_cortex_a8)
    {
...
bfd_boolean
bfd_elf32_arm_allocate_interworking_sections (struct bfd_link_info * hinfo)
{
  struct elf32_arm_link_hash_table * globals;
...
void
bfd_elf32_arm_keep_private_stub_output_sections (struct bfd_link_info *hinfo)
{
  enum elf32_arm_stub_type stub_type;
  ...
  for (stub_type = arm_stub_none + 1; stub_type < max_stub_type; stub_type++)
	...
	continue;
     ...
     out_sec = bfd_get_section_by_name (hinfo->output_bfd, out_sec_name);
     if (out_sec != NULL)
...
bfd_boolean
bfd_elf32_arm_get_bfd_for_interworking (bfd *abfd, struct bfd_link_info *hinfo)
{
  struct elf32_arm_link_hash_table *globals;
...
static int
elf32_thumb_to_arm_stub (struct bfd_link_info * hinfo,
			 const char *		name,
			 bfd *			input_bfd,
			 bfd *			output_bfd,
			 ...
			 char **error_message)
{
  ...
  myh = find_thumb_glue (hinfo, name, error_message);
  if (myh == NULL)
    return FALSE;
...
static struct elf_link_hash_entry *
elf32_arm_create_thumb_stub (struct bfd_link_info * hinfo,
			     const char *	    name,
			     bfd *		    input_bfd,
			     bfd *		    output_bfd,
			     asection *		    sym_sec,
			     ...
			     char **		    error_message)
{
  ...
  myh = find_arm_glue (hinfo, name, error_message);
  if (myh == NULL)
    return NULL;
...
static int
elf32_arm_to_thumb_stub (struct bfd_link_info * hinfo,
			 const char *		name,
			 bfd *			input_bfd,
			 bfd *			output_bfd,
			 asection *		input_section,
			 ...
			 char **error_message)
{
  ...
  myh = elf32_arm_create_thumb_stub (hinfo, name, input_bfd, output_bfd,
...
{
  struct bfd_link_info * hinfo = (struct bfd_link_info *) inf;
  asection * s;
  struct elf_link_hash_entry * myh;
  ...
  myh = elf32_arm_create_thumb_stub (hinfo, h->root.root.string,
...
static bfd_vma
elf32_arm_bx_glue (struct bfd_link_info * hinfo, int reg)
{
  bfd_byte *p;
  bfd_vma glue_addr;
  asection *s;
...
static void
elf32_arm_allocate_dynrelocs (struct bfd_link_info *hinfo, asection *sreloc,
			      bfd_size_type count)
{
  struct elf32_arm_link_hash_table *htab;
...
static void
elf32_arm_allocate_irelocs (struct bfd_link_info *hinfo, asection *sreloc,
			    bfd_size_type count)
{
  struct elf32_arm_link_hash_table *htab;
...
static void
elf32_arm_add_dynreloc (bfd *output_bfd, struct bfd_link_info *hinfo,
			asection *sreloc, Elf_Internal_Rela *rel)
{
  bfd_byte *loc;
  struct elf32_arm_link_hash_table *htab;
...
static void
elf32_arm_allocate_plt_entry (struct bfd_link_info *hinfo,
			      ...
			      struct arm_plt_info *arm_plt)
{
  ...
  if (is_iplt_entry)
	...
	splt->size += htab->plt_header_size;
      ...
      elf32_arm_allocate_irelocs (hinfo, htab->root.irelplt, 1);
    }
  else
    {
      splt = htab->root.splt;
	...
	if (hinfo->flags & DF_BIND_NOW)
	  elf32_arm_allocate_dynrelocs (hinfo, htab->root.srelgot, 1);
	else
	  elf32_arm_allocate_dynrelocs (hinfo, htab->root.srelplt, 1);
      }
    else
      {
  ...
  if (elf32_arm_plt_needs_thumb_stub_p (hinfo, arm_plt))
...
static bfd_boolean
elf32_arm_populate_plt_entry (bfd *output_bfd, struct bfd_link_info *hinfo,
			      union gotplt_union *root_plt,
			      struct arm_plt_info *arm_plt,
			      int dynindx, bfd_vma sym_value)
{
  ...
  if (htab->symbian_p)
  ...
  else
    {
	 ...
	 the same order as in .plt.  */
      ...
      if (htab->vxworks_p && bfd_link_pic (hinfo))
	{
	  unsigned int i;
	  bfd_vma val;
      ...
      else if (htab->nacl_p)
	{
	  ...
	  BFD_ASSERT (!elf32_arm_plt_needs_thumb_stub_p (hinfo, arm_plt));
      ...
      else
	{
      ...
      if (htab->fdpic_p && !(hinfo->flags & DF_BIND_NOW))
	{
  ...
  if (dynindx == -1)
    elf32_arm_add_dynreloc (output_bfd, hinfo, srel, &rel);
  else
    {
      if (htab->fdpic_p)
	{
	  ...
	  if (hinfo->flags & DF_BIND_NOW)
	    elf32_arm_add_dynreloc(output_bfd, hinfo, htab->root.srelgot, &rel);
	  else
	    elf32_arm_add_dynreloc(output_bfd, hinfo, htab->root.srelplt, &rel);
	}
      else
	{
...
static bfd_vma
dtpoff_base (struct bfd_link_info *hinfo)
{
  /* If tls_sec is NULL, we should have signalled an error already.  */
  if (elf_hash_table (hinfo)->tls_sec == NULL)
    return 0;
  return elf_hash_table (hinfo)->tls_sec->vma;
}
...
static bfd_vma
tpoff (struct bfd_link_info *hinfo, bfd_vma address)
{
  struct elf_link_hash_table *htab = elf_hash_table (hinfo);
  bfd_vma base;
			       ...
			       char **			    error_message)
{
  ...
  r_type = elf32_arm_tls_transition (hinfo, r_type, h);
  ...
  if (elf32_arm_get_plt_info (input_bfd, globals, eh, r_symndx, &root_plt,
      ...
      if (h == NULL || eh->is_iplt)
	{
	  ...
	  if (plt_offset & 1)
	    ...
	    {
	      if (elf32_arm_populate_plt_entry (output_bfd, hinfo, root_plt, arm_plt,
						-1, dynreloc_value))
		root_plt->offset |= 1;
	      else
		return bfd_reloc_notsupported;
  ...
  resolved_to_zero = (h != NULL
		      && UNDEFWEAK_NO_DYNAMIC_RELOC (hinfo, h));
  ...
  switch (r_type)
	  ...
	  && plt_offset != (bfd_vma) -1)
	{
	  ...
	  BFD_ASSERT (has_iplt_entry || !SYMBOL_CALLS_LOCAL (hinfo, h));
	  ...
	  && r_type != R_ARM_PLT32)
	{
	  ...
	  *unresolved_reloc_p = FALSE;
	  ...
	  outrel.r_offset =
	    _bfd_elf_section_offset (output_bfd, hinfo, input_section,
				     rel->r_offset);
	  ...
	  if (skip)
	  ...
	  else
	    {
	      ...
	      BFD_ASSERT (r_type == R_ARM_ABS32 || r_type == R_ARM_ABS32_NOI
			  || (globals->fdpic_p && !bfd_link_pic(hinfo)));
		...
		outrel.r_info = ELF32_R_INFO (symbol, R_ARM_IRELATIVE);
	      else if (globals->fdpic_p && !bfd_link_pic(hinfo))
		isrofixup = 1;
	      else
	  ...
	  if (isrofixup)
	    ...
	    elf32_arm_add_dynreloc (output_bfd, hinfo, sreloc, &outrel);
	...
	case R_ARM_PLT32:
	  ...
	  if (r_type == R_ARM_XPC25)
	  ...
	  else if (r_type == R_ARM_PC24)
	    {
	      ...
	      if (branch_type == ST_BRANCH_TO_THUMB)
		{
		  if (elf32_arm_to_thumb_stub (hinfo, sym_name, input_bfd,
	  ...
	  if (   r_type == R_ARM_CALL
	      ...
	      || r_type == R_ARM_PLT32)
	    {
	      ...
	      stub_type = arm_type_of_stub (hinfo, input_section, rel,
					    st_type, &branch_type,
					    hash, value, sym_sec,
					    input_bfd, sym_name);
	...
	else
	  {
		...
		if (globals->use_blx && r_type == R_ARM_THM_CALL)
			 ...
			 && (r_type != R_ARM_THM_JUMP24))
		  {
			...
			(hinfo, sym_name, input_bfd, output_bfd, input_section,
	...
	return bfd_reloc_notsupported;
      ...
      if (dynreloc_st_type == STT_GNU_IFUNC
	  && plt_offset != (bfd_vma) -1
	  && (h == NULL || SYMBOL_REFERENCES_LOCAL (hinfo, h)))
	{
	  ...
	  if ((off & 1) != 0)
	    {
		 ...
		 this symbol.  */
	      off &= ~1;
	      if (globals->root.dynamic_sections_created
		  && !SYMBOL_REFERENCES_LOCAL (hinfo, h))
		*unresolved_reloc_p = FALSE;
	    }
	  else
	    {
		...
		arm_elf_add_rofixup (output_bfd,
				     elf32_arm_hash_table(hinfo)->srofixup,
	...
	if ((off & 1) != 0)
	...
	else
	  {
		...
		bfd_put_32 (output_bfd,
			    !h ? value - elf_hash_table (hinfo)->tls_sec->vma
			    : hinfo->flags & DF_BIND_NOW ? 0
		...
		if (need_relocs)
		  {
		    ...
		    if (indx == 0)
		      bfd_put_32 (output_bfd, value - dtpoff_base (hinfo),
			...
			elf32_arm_add_dynreloc (output_bfd, hinfo,
						srelgot, &outrel);
		      }
		  }
		else
		...
		if (need_relocs)
		  {
		    if (indx == 0)
		      outrel.r_addend = value - dtpoff_base (hinfo);
		    else
		      outrel.r_addend = 0;
		    outrel.r_offset = (sgot->output_section->vma
				       + sgot->output_offset
		  ...
		  bfd_put_32 (output_bfd, tpoff (hinfo, value),
			      sgot->contents + cur_off);
		cur_off += 4;
	      }
	...
	if (ELF32_R_TYPE(rel->r_info) == R_ARM_TLS_CALL
	    || ELF32_R_TYPE(rel->r_info) == R_ARM_THM_TLS_CALL)
	  {
	    ...
	    branch_type = ST_BRANCH_TO_ARM;
	    enum elf32_arm_stub_type stub_type
	      = arm_type_of_stub (hinfo, input_section, rel,
				  st_type, &branch_type,
				  (struct elf32_arm_link_hash_entry *)h,
	  ...
	  if (globals->fix_v4bx == 2 && (insn & 0xf) != 0xf)
	    {
	      /* Branch to veneer.  */
	      bfd_vma glue_addr;
	      glue_addr = elf32_arm_bx_glue (hinfo, insn & 0xf);
	...
	*unresolved_reloc_p = FALSE;
    ...
    case R_ARM_GOTOFFFUNCDESC:
        ...
        if (h == NULL)
	  {
	    ...
	    if (bfd_link_pic(hinfo) && dynindx == 0)
	      abort();
	    ...
	    /* Emit R_ARM_FUNCDESC_VALUE or two fixups on funcdesc if
	       not done yet.  */
	    arm_elf_fill_funcdesc(output_bfd, hinfo,
    ...
    case R_ARM_GOTFUNCDESC:
        ...
        if (h != NULL)
	  {
		...
		outrel.r_offset = sgot->output_section->vma
		  ...
		  + (eh->fdpic_cnts.gotfuncdesc_offset & ~1);
		outrel.r_addend = 0;
		if (h->dynindx == -1 && !bfd_link_pic(hinfo))
		  if (h->root.type == bfd_link_hash_undefweak)
  ...
  for (; rel < relend; rel++)
      ...
      if (r_symndx < symtab_hdr->sh_info)
	{
	  ...
	  if (r_type != R_ARM_V4BX
	    ...
	    (*hinfo->callbacks->undefined_symbol)
	      (hinfo, bfd_elf_string_from_elf_section
      ...
      else
	{
	  ...
	  RELOC_FOR_GLOBAL_SYMBOL (hinfo, input_bfd, input_section, rel,
	...
	RELOC_AGAINST_DISCARDED_SECTION (hinfo, input_bfd, input_section,
	  ...
	  && IS_ARM_TLS_RELOC (r_type) != (sym_type == STT_TLS))
	{
	 ...
	 both in relaxed and non-relaxed cases.  */
      if ((elf32_arm_tls_transition (hinfo, r_type, h) != (unsigned)r_type)
	  || (IS_ARM_TLS_GNU_RELOC (r_type)
      ...
      if (r != bfd_reloc_ok)
	{
	  switch (r)
		...
		(*hinfo->callbacks->reloc_overflow)
		  (hinfo, (h ? &h->root : NULL), name, howto->name,
		   (bfd_vma) 0, input_bfd, input_section, rel->r_offset);
	      break;

	    case bfd_reloc_undefined:
	      (*hinfo->callbacks->undefined_symbol)
		(hinfo, name, input_bfd, input_section, rel->r_offset, TRUE);
	      break;
	    ...
	    common_error:
	      ...
	      (*hinfo->callbacks->reloc_dangerous)
		(hinfo, error_message, input_bfd, input_section, rel->r_offset);
	      break;
	    }
	}
    }
			      ...
			      bfd_boolean merge_exidx_entries)
{
  ...
  /* Walk over all EXIDX sections, and create backlinks from the corrsponding
     text sections.  */
  for (inp = hinfo->input_bfds; inp != NULL; inp = inp->link.next)
    {
      asection *sec;
  ...
  for (i = 0; i < num_text_sections; i++)
      ...
      for (j = 0; j < hdr->sh_size; j += 8)
	{
	  ...
	  if (elide && !bfd_link_relocatable (hinfo))
	    {
  ...
  if (!bfd_link_relocatable (hinfo) && last_exidx_sec
      && last_unwind_type != 0)
    insert_cantunwind_after(last_text_sec, last_exidx_sec);
...
static bfd_boolean
elf32_arm_output_glue_section (struct bfd_link_info *hinfo, bfd *obfd,
			       bfd *ibfd, const char *name)
{
  ...
  if (elf32_arm_write_section (obfd, hinfo, sec, sec->contents))
    return TRUE;
...
#if defined(TARGET_IS_PIC32C)
bfd_boolean pic32c_elf_final_link(bfd *abfd, struct bfd_link_info *hinfo);
#endif

static bfd_boolean
elf32_arm_final_link (bfd *abfd, struct bfd_link_info *hinfo)
{
  struct elf32_arm_link_hash_table *globals = elf32_arm_hash_table (hinfo);
  asection *sec, *osec;
  ...
  if (!bfd_elf_final_link (abfd, hinfo))
    return FALSE;
    
#if defined(TARGET_IS_PIC32C)
  pic32c_elf_final_link(abfd, hinfo);
#endif
  ...
  for (i=0; i<htab->top_id; i++)
	...
	{
	  osec = sec->output_section;
	  elf32_arm_write_section (abfd, hinfo, sec, sec->contents);
  ...
  if (globals->bfd_of_glue_owner != NULL)
    {
      if (! elf32_arm_output_glue_section (hinfo, abfd,
...
static bfd_boolean
elf32_arm_merge_eabi_attributes (bfd *ibfd, struct bfd_link_info *hinfo)
{
  bfd *obfd = hinfo->output_bfd;
...
#define VFP_VERSION_COUNT 9
  ...
  if (!_bfd_elf_merge_object_attributes (ibfd, hinfo))
    return FALSE;
			...
			Elf_Internal_Sym *sym)
{
  ...
  return _bfd_elf_gc_mark_hook (sec, hinfo, rel, h, sym);
}

/* Look through the relocs for a section during the first phase.  */

static bfd_boolean
elf32_arm_check_relocs (bfd *abfd, struct bfd_link_info *hinfo,
			asection *sec, const Elf_Internal_Rela *relocs)
{
  ...
  if (htab->root.is_relocatable_executable
    ...
    {
      if (! _bfd_elf_link_create_dynamic_sections (abfd, hinfo))
	return FALSE;
    }

  if (htab->root.dynobj == NULL)
    htab->root.dynobj = abfd;
  if (!create_ifunc_sections (hinfo))
    return FALSE;
  ...
  for (rel = relocs; rel < rel_end; rel++)
	...
	case R_ARM_FUNCDESC:
	  ...
	  case R_ARM_THM_TLS_CALL:
		...
		default: tls_type = GOT_NORMAL; break;
	      ...
	      if (!bfd_link_executable (hinfo) && (tls_type & GOT_TLS_IE))
		hinfo->flags |= DF_STATIC_TLS;
	  ...
	  case R_ARM_GOTPC:
		...
		&& !create_got_section (htab->root.dynobj, hinfo))
	      return FALSE;
	    break;
	  ...
	  case R_ARM_ABS32_NOI:
	jump_over:
	    if (h != NULL && bfd_link_executable (hinfo))
	      {
	  ...
	  case R_ARM_THM_MOVT_PREL:
	    ...
	    if ((bfd_link_pic (hinfo) || htab->root.is_relocatable_executable
		 || htab->fdpic_p)
      ...
      if (may_become_dynamic_p)
	{
	  ...
	  if (elf32_arm_howto_from_type (r_type)->pc_relative)
	    p->pc_count += 1;
	  p->count += 1;
	  if (h == NULL && htab->fdpic_p && !bfd_link_pic(hinfo)
...
static bfd_boolean
elf32_arm_gc_mark_extra_sections (struct bfd_link_info *hinfo,
				  elf_gc_mark_hook_fn gc_mark_hook)
{
  ...
  _bfd_elf_gc_mark_extra_sections (hinfo, gc_mark_hook);

  out_attr = elf_known_obj_attributes_proc (hinfo->output_bfd);
  is_v8m = out_attr[Tag_CPU_arch].i >= TAG_CPU_ARCH_V8M_BASE
	   && out_attr[Tag_CPU_arch_profile].i == 'M';
  ...
  while (again)
    {
      again = FALSE;
      for (sub = hinfo->input_bfds; sub != NULL; sub = sub->link.next)
	{
	  asection *o;
	  ...
	  for (o = sub->sections; o != NULL; o = o->next)
		...
		{
		  again = TRUE;
		  if (!_bfd_elf_gc_mark (hinfo, o, gc_mark_hook))
		    return FALSE;
		}
	    }
		  ...
		  if (ARM_GET_SYM_CMSE_SPCL (cmse_hash->root.target_internal))
		    {
		      ...
		      if (!cmse_sec->gc_mark
			  && !_bfd_elf_gc_mark (hinfo, cmse_sec, gc_mark_hook))
			return FALSE;
...
static bfd_boolean
elf32_arm_adjust_dynamic_symbol (struct bfd_link_info * hinfo,
				 struct elf_link_hash_entry * h)
{
  bfd * dynobj;
  asection *s, *srel;
  ...
  dynobj = elf_hash_table (hinfo)->dynobj;
  ...
  /* If we are creating a shared library, we must presume that the
     ...
     do anything here.  */
  if (bfd_link_pic (hinfo) || globals->root.is_relocatable_executable)
    return TRUE;
  ...
  if (hinfo->nocopyreloc == 0
      && (h->root.u.def.section->flags & SEC_ALLOC) != 0
      && h->size != 0)
    {
      elf32_arm_allocate_dynrelocs (hinfo, srel, 1);
      h->needs_copy = 1;
    }

  return _bfd_elf_adjust_dynamic_copy (hinfo, h, s);
}
...
allocate_dynrelocs_for_symbol (struct elf_link_hash_entry *h, void * inf)
{
  ...
  hinfo = (struct bfd_link_info *) inf;
  htab = elf32_arm_hash_table (hinfo);
  if (htab == NULL)
    return FALSE;
  ...
  if ((htab->root.dynamic_sections_created || h->type == STT_GNU_IFUNC)
      && h->plt.refcount > 0)
    {
	 ...
	 Undefined weak syms won't yet be marked as dynamic.  */
	  ...
	  && h->root.type == bfd_link_hash_undefweak)
	{
	  if (! bfd_elf_link_record_dynamic_symbol (hinfo, h))
	    return FALSE;
	}
	 ...
	 than the .plt section.  */
      if (h->type == STT_GNU_IFUNC && SYMBOL_CALLS_LOCAL (hinfo, h))
	{
	  eh->is_iplt = 1;
	...
	{
	  elf32_arm_allocate_plt_entry (hinfo, eh->is_iplt, &h->plt, &eh->plt);
  ...
  if (h->got.refcount > 0)
      ...
      if (!htab->symbian_p)
	{
	  ...
	  if (tls_type != GOT_NORMAL
	    ...
	    elf32_arm_allocate_irelocs (hinfo, htab->root.srelgot, 1);
	  else if (bfd_link_pic (hinfo)
		   && (ELF_ST_VISIBILITY (h->other) == STV_DEFAULT
		       || h->root.type != bfd_link_hash_undefweak))
  ...
  if (eh->fdpic_cnts.funcdesc_cnt > 0)
      ...
      else
	{
	  ...
	  elf32_arm_allocate_dynrelocs (hinfo, htab->root.srelgot,
					eh->fdpic_cnts.funcdesc_cnt);
	}
    }
  ...
  if (!htab->use_blx && h->dynindx != -1
      ...
      && ELF_ST_VISIBILITY (h->other) == STV_DEFAULT)
    {
      ...
      _bfd_generic_link_add_one_symbol (hinfo, s->owner,
					name, BSF_GLOBAL, s,
					h->root.u.def.value,
					NULL, TRUE, FALSE, &bh);
      ...
      eh->export_glue = myh;
      th = record_arm_to_thumb_glue (hinfo, h);
  ...
  if (bfd_link_pic (hinfo) || htab->root.is_relocatable_executable || htab->fdpic_p)
    {
  ...
  for (p = eh->dyn_relocs; p != NULL; p = p->next)
    {
	  ...
	  && eh->plt.noncall_refcount == 0
	  && SYMBOL_REFERENCES_LOCAL (hinfo, h))
	elf32_arm_allocate_irelocs (hinfo, sreloc, p->count);
      else if (h->dynindx != -1 && (!bfd_link_pic(hinfo) || !hinfo->symbolic || !h->def_regular))
	elf32_arm_allocate_dynrelocs (hinfo, sreloc, p->count);
      else if (htab->fdpic_p && !bfd_link_pic(hinfo))
...
maybe_set_textrel (struct elf_link_hash_entry *h, void *info_p)
{
  ...
  if (sec != NULL)
    {
      struct bfd_link_info *hinfo = (struct bfd_link_info *) info_p;

      hinfo->flags |= DF_TEXTREL;
      hinfo->callbacks->minfo
...
void
bfd_elf32_arm_set_byteswap_code (struct bfd_link_info *hinfo,
				 int byteswap_code)
{
  struct elf32_arm_link_hash_table *globals;
				 ...
				 struct bfd_link_info * hinfo)
{
  ...
  if (elf_hash_table (hinfo)->dynamic_sections_created)
    {
      /* Set the contents of the .interp section to the interpreter.  */
      if (bfd_link_executable (hinfo) && !hinfo->nointerp)
	{
  ...
  /* Set up .got offsets for local syms, and space for local dynamic
     relocs.  */
  for (ibfd = hinfo->input_bfds; ibfd != NULL; ibfd = ibfd->link.next)
    {
      ...
      for (s = ibfd->sections; s != NULL; s = s->next)
	{
	  ...
	  for (p = (struct elf_dyn_relocs *)
	      ...
	      else if (p->count != 0)
		{
		  ...
		  if (htab->fdpic_p && !bfd_link_pic(hinfo))
		    htab->srofixup->size += 4 * p->count;
		  else
		    elf32_arm_allocate_dynrelocs (hinfo, srel, p->count);
	  ...
	  *local_tlsdesc_gotent = (bfd_vma) -1;
	  ...
	  if (local_fdpic_cnts->funcdesc_cnt > 0)
	      ...
	      if (local_fdpic_cnts->funcdesc_offset == -1)
		{
	      ...
	      if (bfd_link_pic(hinfo))
		elf32_arm_allocate_dynrelocs (hinfo, srel, local_fdpic_cnts->funcdesc_cnt);
	      else
	  ...
	  if (local_iplt != NULL)
		...
		{
		  elf32_arm_allocate_plt_entry (hinfo, TRUE,
						&local_iplt->root,
						&local_iplt->arm);
	      ...
	      for (p = local_iplt->dyn_relocs; p != NULL; p = p->next)
		{
		  ...
		  if (local_iplt->arm.noncall_refcount == 0)
		    elf32_arm_allocate_irelocs (hinfo, psrel, p->count);
		  else
		    elf32_arm_allocate_dynrelocs (hinfo, psrel, p->count);
		}
	    }
	  if (*local_got > 0)
	    {
		...
		elf32_arm_allocate_irelocs (hinfo, srel, 1);
	      else if (bfd_link_pic (hinfo) || output_bfd->flags & DYNAMIC || htab->fdpic_p)
		{
		  if ((bfd_link_pic (hinfo) && !(*local_tls_type & GOT_TLS_GDESC)))
		    elf32_arm_allocate_dynrelocs (hinfo, srel, 1);
		  else if (htab->fdpic_p && *local_tls_type & GOT_NORMAL)
		    htab->srofixup->size += 4;

		  if ((bfd_link_pic (hinfo) || htab->fdpic_p)
		      && *local_tls_type & GOT_TLS_GDESC)
		    {
		      elf32_arm_allocate_dynrelocs (hinfo,
						    htab->root.srelplt, 1);
		      htab->tls_trampoline = -1;
		    }
		}
  ...
  /* Allocate global sym .plt and .got entries, and space for global
     sym dynamic relocs.  */
  elf_link_hash_traverse (& htab->root, allocate_dynrelocs_for_symbol, hinfo);
  ...
  for (ibfd = hinfo->input_bfds; ibfd != NULL; ibfd = ibfd->link.next)
	...
	continue;
      ...
      if (!bfd_elf32_arm_process_before_allocation (ibfd, hinfo)
	  || !bfd_elf32_arm_vfp11_erratum_scan (ibfd, hinfo)
	  || !bfd_elf32_arm_stm32l4xx_erratum_scan (ibfd, hinfo))
	_bfd_error_handler (_("errors encountered processing file %pB"), ibfd);
    }

  /* Allocate space for the glue sections now that we've sized them.  */
  bfd_elf32_arm_allocate_interworking_sections (hinfo);
...
#define add_dynamic_entry(TAG, VAL) \
  _bfd_elf_add_dynamic_entry (hinfo, TAG, VAL)
      ...
      if (relocs)
	{
	 ...
	 then we need a DT_TEXTREL entry.  */
      if ((hinfo->flags & DF_TEXTREL) == 0)
	elf_link_hash_traverse (&htab->root, maybe_set_textrel, hinfo);
      ...
      if ((hinfo->flags & DF_TEXTREL) != 0)
	{
      ...
      if (htab->vxworks_p
	  && !elf_vxworks_add_dynamic_entries (output_bfd, hinfo))
	return FALSE;
    }
				...
				struct bfd_link_info *hinfo)
{
  ...
  tls_sec = elf_hash_table (hinfo)->tls_sec;
  ...
  if (tls_sec)
	...
	(elf_hash_table (hinfo), "_TLS_MODULE_BASE_", TRUE, TRUE, FALSE);
      ...
      if (tlsbase)
	{
	  ...
	  if (!(_bfd_generic_link_add_one_symbol
		(hinfo, output_bfd, "_TLS_MODULE_BASE_", BSF_LOCAL,
	  ...
	  (*bed->elf_backend_hide_symbol) (hinfo, tlsbase, TRUE);
	}
    }

  if (htab->fdpic_p && !bfd_link_relocatable (hinfo)
      && !bfd_elf_stack_segment_size (output_bfd, hinfo,
				 ...
				 Elf_Internal_Sym * sym)
{
  ...
  if (h->plt.offset != (bfd_vma) -1)
      ...
      if (!eh->is_iplt)
	{
	  ...
	  if (! elf32_arm_populate_plt_entry (output_bfd, hinfo, &h->plt, &eh->plt,
					      h->dynindx, 0))
	    return FALSE;
	}
  ...
  if (h->needs_copy)
	...
	s = htab->root.srelbss;
      elf32_arm_add_dynreloc (output_bfd, hinfo, s, &rel);
    }
...
static bfd_boolean
elf32_arm_finish_dynamic_sections (bfd * output_bfd, struct bfd_link_info * hinfo)
{
  bfd * dynobj;
  asection * sgot;
  asection * sdyn;
  ...
  if (elf_hash_table (hinfo)->dynamic_sections_created)
      ...
      for (; dyncon < dynconend; dyncon++)
	{
	  ...
	  switch (dyn.d_tag)
	    ...
	    case DT_INIT:
	      name = hinfo->init_function;
	      goto get_sym;
	    case DT_FINI:
	      name = hinfo->fini_function;
	    get_sym:
	      ...
	      if (dyn.d_un.d_val != 0)
		{
		  ...
		  eh = elf_link_hash_lookup (elf_hash_table (hinfo), name,
					     FALSE, FALSE, TRUE);
		  if (eh != NULL
...
elf32_arm_reloc_type_class (const struct bfd_link_info *hinfo ATTRIBUTE_UNUSED,
			    ...
			    struct arm_plt_info *arm_plt)
{
  ...
  htab = elf32_arm_hash_table (osi->hinfo);
  if (htab == NULL)
    return FALSE;
  ...
  osi->sec_shndx = (_bfd_elf_section_from_bfd_section
		    (osi->hinfo->output_bfd, osi->sec->output_section));
  ...
  if (htab->symbian_p)
	...
	: ARM_MAP_ARM;
      ...
      if (elf32_arm_plt_needs_thumb_stub_p (osi->hinfo, arm_plt))
  ...
  else
    {
      ...
      thumb_stub_p = elf32_arm_plt_needs_thumb_stub_p (osi->hinfo, arm_plt);
      if (thumb_stub_p)
	{
	  if (!elf32_arm_output_map_sym (osi, ARM_MAP_THUMB, addr - 4))
	    return FALSE;
...
elf32_arm_output_plt_map (struct elf_link_hash_entry *h, void *inf)
{
  ...
  eh = (struct elf32_arm_link_hash_entry *) h;
  return elf32_arm_output_plt_map_1 (osi, SYMBOL_CALLS_LOCAL (osi->hinfo, h),
				     &h->plt, &eh->plt);
}
					       ...
					       struct elf_link_hash_entry *))
{
  ...
  /* Add a $d mapping symbol to data-only sections that
     ...
     mapping symbols.  */
  for (input_bfd = hinfo->input_bfds;
       input_bfd != NULL;
       input_bfd = input_bfd->link.next)
    {
				 ...
				 asymbol **syms, long symcount)
{
  ...
  BFD_ASSERT (!(bfd_get_file_flags (hinfo->out_implib_bfd) & EXEC_P));
  if (globals->cmse_implib)
    return elf32_arm_filter_cmse_symbols (abfd, hinfo, syms, symcount);
  else
    return _bfd_elf_filter_global_symbols (abfd, hinfo, syms, symcount);
}
...
#ifdef TARGET_IS_PIC32C
    ///\ set segment flags
    ///\ added for pic32c, where there are no readonly sections
    m->p_flags = PF_R;
    m->p_flags_valid = 1;
#endif
    }
...
static bfd_boolean
elf32_arm_add_symbol_hook (bfd *abfd, struct bfd_link_info *hinfo,
			   Elf_Internal_Sym *sym, const char **namep,
			   flagword *flagsp, asection **secp, bfd_vma *valp)
{
  if (elf32_arm_hash_table (hinfo) == NULL)
    return FALSE;

  if (elf32_arm_hash_table (hinfo)->vxworks_p
      && !elf_vxworks_add_symbol_hook (abfd, hinfo, sym, namep,
				       flagsp, secp, valp))
    return FALSE;
...
#if defined(TARGET_IS_PIC32C)
/* MERGE-TODO: unify attributes? */
#include "pic32c-attributes.c"
#endif
...
static bfd_boolean
elf32_arm_nacl_modify_segment_map (bfd *abfd, struct bfd_link_info *hinfo)
{
  return (elf32_arm_modify_segment_map (abfd, hinfo)
	  && nacl_modify_segment_map (abfd, hinfo));
}
...
static bfd_boolean
elf32_arm_merge_private_bfd_data (bfd *ibfd, struct bfd_link_info *hinfo)
{
  bfd *obfd = hinfo->output_bfd;
  flagword out_flags;
  flagword in_flags;
  ...
  if (! _bfd_generic_verify_endian_match (ibfd, hinfo))
    return FALSE;

  if (! is_arm_elf (ibfd) || ! is_arm_elf (obfd))
    return TRUE;

  if (!elf32_arm_merge_eabi_attributes (ibfd, hinfo))
    return FALSE;
