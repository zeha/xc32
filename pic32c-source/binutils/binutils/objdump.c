...
#define PF2(x, y) \
  ...
  PF2 (stack, "STACK");
  PF2 (ramfunc, "RAMFUNC");
  /* MERGE-TODO: check/enable coherent */
  ...
  while (addr_offset < stop_offset)
    {
      ...
      /* This is guarded because we similarly guard the only use below. */
...
#if defined(TARGET_MCHP_PIC32C) || defined(TARGET_MCHP_PIC32MX)
extern const char *const SU_MEMTYPE_REPR[];
extern const char *const SU_FLAGS_REPR[];

static void disasm_su_hdr_section(bfd *abfd, asection *section)
{
  int i = 0;
  bfd_byte *data = NULL;
  ...
  printf("XC32 Stack Usage Version: 0x%X\n", data[1]);

  free(data);
}

static void disasm_su_section(bfd *abfd, asection *section)
{
  bfd_byte *data = NULL;
  bfd_size_type datasize;
  int i, fname_sz;
  ...
  while (offset < datasize)
    {
      int first_flag = 1;
      uint32_t faddr = value32_to_host(abfd, *(uint32_t *)(data + offset));
      offset += 4;
      char *fname = (char *)(data + offset);
      fname_sz = strlen(fname);
      offset += (fname_sz + 1);
      uint32_t size = value32_to_host(abfd, *(uint32_t *)(data + offset));
      offset += 4;
      uint32_t flags = value32_to_host(abfd, *(uint32_t *)(data + offset));
      offset += 4;
      uint8_t kind = data[offset];
      offset++;
      printf("  <%s>:\n", fname);
      printf("\tused_stack: 0x%X\n", size);
            ...
            if (flags & (1 << i))
              {
                ...
                if (SU_INTERRUPT == (1 << i))
                  {
                    int ipl = SU_GET_IPL(flags);
                    printf("%s:ipl%d", SU_FLAGS_REPR[i], ipl);
                  }
                else
                  {
...
#if defined(TARGET_MCHP_PIC32C) || defined(TARGET_MCHP_PIC32MX)
  if (strcmp(sanitize_string(section->name), SU_SECTION_HDR) == 0)
    {
      disasm_su_hdr_section(abfd, section);
      return;
    }
  else if (strcmp(sanitize_string(section->name), SU_SECTION) == 0)
    {
      disasm_su_section(abfd, section);
      return;
    }
#endif
