...
#if 0
#ifdef TARGET_IS_PIC32C
          /* SIO_TODO: figure out which symbols to keep here now that we 
             are properly pulling in undefined ones above. */
          if (mchp_elf_link_check_archive_element &&
              !mchp_elf_link_check_archive_element (symdef->name, element, info))
            {
              continue;
            }
#endif
...
#if defined(TARGET_IS_PIC32C) || defined(TARGET_IS_PIC32M)
