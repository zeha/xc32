...
#
# Special variables for PIC32C support:
#       TEXT_REGION - name for text memory region
#       RODATA_REGION - name for read-only data region (default TEXT_REGION)
#       DATA_REGION - name for data memory region
#       BSS_REGION - name  for BSS memory region (defaults to DATA_REGION)
#       ENABLE_TCM - if 'yes' emit TCM memory region names
#       *_START_ADDR, *_LENGTH - parameters for memory regions
#         TEXT, DATA, TEXT_TCM, DATA_TCM
...
test -z "$TEXT_REGION" && TEXT_REGION=rom
test -z "$RODATA_REGION" && RODATA_REGION=${TEXT_REGION}
test -z "$DATA_REGION" && DATA_REGION=ram
test -z "$BSS_REGION" && BSS_REGION=${DATA_REGION}

if [ "x${ENABLE_TCM}" == "xyes" ]; then
  test -z "$TEXT_TCM_REGION" && TEXT_TCM_REGION=itcm
  test -z "$DATA_TCM_REGION" && DATA_TCM_REGION=dtcm
fi

# Start stuff inherited from elf.sc
...
DISCARDED="/DISCARD/ : { *(.note.GNU-stack) *(.gnu_debuglink) *(.gnu.lto_*) *(.gnu.warning.*) }"
...
# Start emitting the script here..

if [ "x$ENABLE_TCM" == "xyes" ]; then
  TCM_REGIONS="
     ${TEXT_TCM_REGION} (RX) : ORIGIN = ${TEXT_TCM_START_ADDR}, LENGTH = ${TEXT_TCM_LENGTH}
     ${DATA_TCM_REGION} (WX!R) : ORIGIN = ${DATA_TCM_START_ADDR}, LENGTH = ${DATA_TCM_LENGTH}
   "
else
  TCM_REGIONS=""
fi

# This allows for other named regions for BSS, RODATA. Currently not supported.
OTHER_REGIONS=""

cat <<EOF
MEMORY
{
  ${TEXT_REGION} (LRX) : ORIGIN = ${TEXT_START_ADDR}, LENGTH = ${TEXT_LENGTH}
  ${DATA_REGION} (WX!R) : ORIGIN = ${DATA_START_ADDR}, LENGTH = ${DATA_LENGTH} - (${TEXT_TCM_LENGTH} + ${DATA_TCM_LENGTH})
  ${OTHER_REGIONS}
  ${TCM_REGIONS}
}
...
cat <<EOF
  ...
  .${RODATA_NAME}1      ${RELOCATING-0} : { *(.${RODATA_NAME}1) } >${RODATA_REGION}
  ...
  .data1        ${RELOCATING-0} : { *(.data1) } >${DATA_REGION}
  ${WRITABLE_RODATA+${RODATA}}
