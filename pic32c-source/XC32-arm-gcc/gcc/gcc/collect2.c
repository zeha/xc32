...
#ifdef _WIN32
/* Undo the effect of convert_white_space() in gcc.c on the value of the
 * COLLECT_LTO_WRAPPER environment variable (the backslashes cause issues
 * on Windows).
 * NOTE: Since the environment variable aren't supposed to be directly
 * modified, we always return a (modified or not) copy that ultimately
 * must be free()'d; orig is assumed to be non-NULL by this function */
static char *
undo_convert_white_space (char *orig)
{
    /* The destination string can be at most as long as the original one */
    char *const dest = (char *) xmalloc (strlen (orig) + 1), *d = dest;

    /* Copy the characters from the original string to the destination
     * one, replacing each '\\', ' ' sequence with a single ' ' */ 
    while (const char c = *orig++)
        *d++ = (c != '\\' || *orig != ' ') ? c : (++orig, ' ');
...
#ifdef _WIN32
      /* On Windows, undo the space escaping done by convert_white_space()
       * from gcc.c when setting the COLLECT_LTO_WRAPPER env var */
      lto_wrapper = undo_convert_white_space (lto_wrapper);
#endif
...
#ifdef _WIN32
      /* On Windows, this is heap-allocated so don't let it leak */
      free (lto_wrapper);
#endif
    }
  else if (force)
