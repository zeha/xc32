...
#ifdef _BUILD_MCHP_
# if _BUILD_MCHP_DEVEL_ && !defined(ENABLE_RTL_TRACE)
#   define ENABLE_RTL_TRACE 1
# endif
#endif

/* When ENABLE_RTL_TRACE is set, define TRACE_RTL and TRACE_RTL_DF to
   print a stack trace on each insn emitted when dump_file is non-NULL
   and -ftrace-emit-rtl is specified.
   */
#ifdef ENABLE_RTL_TRACE

#include <execinfo.h>

/* Maximum trace depth */
#ifndef RTL_TRACE_DEPTH
#define RTL_TRACE_DEPTH 16
#endif

#define TRACE_RTL(insn, fp)                                                    \
  do {                                                                         \
    if (flag_trace_emit_rtl)                                                   \
      rtl_trace(fp, insn);                                                     \
  } while (0)

#define TRACE_RTL_DF(insn)                                                     \
  do {                                                                         \
    if (flag_trace_emit_rtl && dump_file)                                      \
      rtl_trace(dump_file, insn);                                              \
  } while (0)

static void 
rtl_trace(FILE *fp, rtx insn, const char *annot = NULL)
{
  void *callstack[RTL_TRACE_DEPTH];
  int i, frames;
  char **strs;

  frames = backtrace (callstack, RTL_TRACE_DEPTH);
  strs = backtrace_symbols (callstack, frames);

  fprintf (dump_file, "insn::\n");
  print_rtl_single (fp, insn);
  fprintf (dump_file, "\n");
  fprintf (fp, "generated from:\n");
  for (i = 0; i < frames; ++i) 
    {
      fprintf (fp, "    %s\n", strs[i]);
    }
...
make_debug_insn_raw (rtx pattern)
{
  ...
  /* debug insns cause backtrace to fail on opening the executable
     so we do not trace back here */
  return insn;
}
