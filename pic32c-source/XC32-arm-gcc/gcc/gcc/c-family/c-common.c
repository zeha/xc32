...
#ifdef _BUILD_MCHP_
#include "tm.h" /* for build_smartio_format */
#endif
...
static void
check_nonnull_arg (void *ctx, tree *param_p, unsigned HOST_WIDE_INT param_num)
{
  tree param = *param_p;
...
/* SIO_TODO: the callback must take a pointer to the tree parameter as
   smart-io requires that we modify it. */
void
check_function_arguments_recurse (void (*callback)
				  (void *, tree *, unsigned HOST_WIDE_INT),
				  void *ctx, tree *param_p,
				  unsigned HOST_WIDE_INT param_num)
{
      ...
      if (SMARTIO_FORMAT_P (TREE_OPERAND (param, 0)))
        {
          ...
          tree fmt = TREE_VALUE (TREE_OPERAND (param, 0));
          tree spec = TREE_VALUE (TREE_CHAIN (TREE_OPERAND (param, 0)));

          TREE_OPERAND (param, 0) = fmt;
          *param_p = build_smartio_format (param, spec);
        }
#endif
      return;
    }
	...
	if (is_attribute_p ("format_arg", TREE_PURPOSE (attrs)))
	  {
	      ...
	      if (i == format_num)
		{
		  check_function_arguments_recurse (callback, ctx,
						    &inner_arg, param_num);
#ifdef _BUILD_MCHP_
                  /* smart-io: strip the format if it's within a call, we
                     can't handle it. */
                  if (SMARTIO_FORMAT_P (inner_arg))
                    inner_arg = TREE_VALUE (inner_arg);
#endif
...
#ifdef _BUILD_MCHP_
	  /* smart-io: handle conditional formats by ensuring
	     equality of conversion spec sets and propagating that
	     down to the root of the conditional.
	     In the future we can consider taking the union. */
	  if (SMARTIO_FORMAT_P (TREE_OPERAND (param, 1))
	      || SMARTIO_FORMAT_P (TREE_OPERAND (param, 2)))
	    {
              tree lhs = TREE_OPERAND (param, 1);
              tree rhs = TREE_OPERAND (param, 2);

              /* If either did not get a smartio-fmt, strip the other. */
              if (!SMARTIO_FORMAT_P (lhs))
                TREE_OPERAND (param, 2) = TREE_VALUE (rhs);
              else if (!SMARTIO_FORMAT_P (rhs))
                TREE_OPERAND (param, 1) = TREE_VALUE (lhs);
	      else
		{
		  /* Both are smartio-fmt. Ensure they match and make the spec
		     apply to the conditional expr. */
		  gcc_assert (SMARTIO_SPEC_P (TREE_CHAIN (lhs)));
		  gcc_assert (SMARTIO_SPEC_P (TREE_CHAIN (rhs)));

		  if (!strcmp (SMARTIO_SPEC_STRING (lhs),
			       SMARTIO_SPEC_STRING (rhs)))
		    {
		      *param_p
			= build_smartio_format (param, SMARTIO_SPEC (lhs));
		      TREE_OPERAND (param, 1) = SMARTIO_FORMAT (lhs);
		      TREE_OPERAND (param, 2) = SMARTIO_FORMAT (rhs);
		    }
		}
	    }
#endif
  ...
  (*callback) (ctx, param_p, param_num);
}
