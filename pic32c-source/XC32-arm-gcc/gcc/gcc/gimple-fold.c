...
/* Simplify a call to the sprintf builtin with arguments DEST, FMT, and ORIG.
   ...
   pos indicates the position of first argument to consider, which will generally
   be 0. For smart-io builtins, this will be the first argument after the 
   format parameter.
*/

bool
gimple_fold_builtin_sprintf (gimple_stmt_iterator *gsi, int pos)
{
  gimple *stmt = gsi_stmt (*gsi);
  tree dest = gimple_call_arg (stmt, pos+0);
  tree fmt = gimple_call_arg (stmt, pos+1);
  tree orig = NULL_TREE;
  ...
  /* Verify the required arguments in the original call.  We deal with two
     ...
     'sprintf (dest, "%s", orig)'.  */
  if (gimple_call_num_args (stmt) > (pos+3))
    return false;

  if (gimple_call_num_args (stmt) == (pos+3))
    orig = gimple_call_arg (stmt, pos+2);
...
bool
gimple_fold_builtin_snprintf (gimple_stmt_iterator *gsi, int pos)
{
  gcall *stmt = as_a <gcall *> (gsi_stmt (*gsi));
  tree dest = gimple_call_arg (stmt, pos+0);
  tree destsize = gimple_call_arg (stmt, pos+1);
  tree fmt = gimple_call_arg (stmt, pos+2);
  tree orig = NULL_TREE;
  const char *fmt_str = NULL;

  if (gimple_call_num_args (stmt) > (pos+4))
    return false;

  if (gimple_call_num_args (stmt) == (pos+4))
    orig = gimple_call_arg (stmt, pos+3);
