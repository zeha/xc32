...
/* MCHP only: return block size estimate. */
static void
rtl_dump_bb (FILE *outf, basic_block bb, int indent, int flags)
{
  ...
  rtx_insn *last;
  char *s_indent;
  unsigned long insn_count = 0, insn_size = 0;
...
#if defined(_BUILD_MCHP_) && _BUILD_MCHP_DEVEL_
        if ((flags & TDF_CODESIZE) && INSN_P (insn))
          {
            ++insn_count;
	    if (!JUMP_P (insn))
	      insn_size += get_attr_length (insn);
	  }
#endif
      }

#if defined(_BUILD_MCHP_) && _BUILD_MCHP_DEVEL_
  if (flags & TDF_CODESIZE)
    {
      fprintf (outf, "estimated block count: %ld\n", insn_count);
      fprintf (outf, "estimated block size: %ld\n", insn_size);
    }
#endif
...
print_rtl_with_bb (FILE *outf, const rtx_insn *rtx_first, int flags)
{
  ...
  unsigned long insn_count = 0, insn_size = 0, bb_insn_count = 0,
		bb_insn_size = 0;
...
#if defined(_BUILD_MCHP_) && _BUILD_MCHP_DEVEL_
          if ((flags & TDF_CODESIZE) && INSN_P (tmp_rtx))
            {
              ++bb_insn_count;
              ++insn_count;
            }
...
#if defined(_BUILD_MCHP_) && _BUILD_MCHP_DEVEL_
      if (flags & TDF_CODESIZE)
        fprintf (outf, "estimated function count: %ld\n", insn_count);
#endif
      free (start);
      free (end);
      free (in_bb_p);
