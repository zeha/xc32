...
/* PIC32: see comment in obj-elf.h at declaration. */
void
...
#ifdef _BUILD_MCHP_
  /* existing_sec is set when called from our change_section hooks. this means
     we've already created the section, massaged various attributes, and are now handing
     off to the generic hook to handle the rest. */
  else if (existing_sec)  
    {
      sec = existing_sec;
      subseg_set (sec, 0);
    }
  ...
  if (old_sec == NULL)
      ...
      if (attr != 0)
	{
	  ...
	  if (((old_sec->flags ^ flags)
		  ...
		  | SEC_THREAD_LOCAL))
#ifdef _BUILD_MCHP_
              /* we must expect changes when called from our section change
                 hooks (existing_sec indicates this case). */
               && ! existing_sec
#endif
              )
            {
...
#if defined(TARGET_IS_PIC32MX) || defined(TARGET_IS_PIC32C)
  extern int pic32_gen_stack_usage;
  extern void pic32_su_start_function (const char *);
  if ((pic32_gen_stack_usage != 0) && (type & BSF_FUNCTION) != 0)
