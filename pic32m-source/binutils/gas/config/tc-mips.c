...
/* Enable warnings for deprecated features. Basically just here to allow 
   the quoted section flags warnings to be silenced. */
static int mips_warn_deprecated = 0;
#endif
...
enum options
  {
    ...
    OPTION_WARN_DEPRECATED,
    OPTION_NO_WARN_DEPRECATED,
#if defined(TARGET_IS_PIC32MX)
    OPTION_STACK_USAGE,
#endif
    OPTION_END_OF_ENUM
...
#ifdef TARGET_IS_PIC32MX
  /* Reuse mwarn-deprecated from ARM to guard the quoted flags warnings. */
  {"mwarn-deprecated", no_argument, NULL, OPTION_WARN_DEPRECATED},
  {"mno-warn-deprecated", no_argument, NULL, OPTION_NO_WARN_DEPRECATED},
  {"mchp-stack-usage", no_argument, NULL, OPTION_STACK_USAGE},
#endif
...
#if defined(TARGET_IS_PIC32MX)
  if (pic32_gen_stack_usage == STACK_USAGE_ENABLED && !insn_error.msg)
    pic32_su_handle_mips_insn (first, tokens);
#endif
...
#ifdef TARGET_IS_PIC32MX
    case OPTION_WARN_DEPRECATED:
      mips_warn_deprecated = TRUE;
      break;

    case OPTION_NO_WARN_DEPRECATED:
      mips_warn_deprecated = FALSE;
      break;

    case OPTION_STACK_USAGE:
...
#error "tc-mips.c requires DEFINED_SECTION_STACK"
#endif
...
s_change_section (int push)
{
  ...
  /* this code tries to look ahead for things that aren't attributes,
   ...
   if (has_flags) {
     ...
     if (!flag_no_warnings && mips_warn_deprecated)
   ...
   /* new style section directive */
     ...
     if ((c == '*') && (strlen(section_name) == 0))
       {
         ...
         if (strlen(p) > 0) {  /* if a name is provided, use it */
           char *tmp_name = xmalloc (input_line_pointer - p + 1);
           strcpy (tmp_name, p);
           name = tmp_name;
         } else {              /* else invent a name */
           as_where(&lineno);
	   /* MERGE-NOTES: filename is uninitialized here. Am initializing
	      to the empty string as a sane default. */
           len = strlen(filename) + strlen(s) + 5;
           char *tmp_name = xmalloc (len + 1);
           snprintf(tmp_name, len, "%s%s%d", filename, s, ++auto_name_cnt);
           name = tmp_name;
         }
       ...
       char *tmp_name = xmalloc (input_line_pointer - section_name + 1);
       strcpy (tmp_name, section_name);
       name = tmp_name;
       /*xc32-129*/
     ...
     if (pic32_extended_attribute_map(sec)) {
      char *mangled_name;
      const char *ext_attr_prefix = "__ext_attr_";
      symbolS * symbolp;
     ...
     if ( PIC32_IS_BSS_ATTR(sec) || PIC32_IS_BSS_ATTR_WITH_MEMORY_ATTR(sec))
      ...
      /* BSS/PERSIST sections are SHF_ALLOC with SHT_NOBITS. elf_section_type doesn't
         know about these attributes, so we clean it up here. */
      if (PIC32_IS_BSS_ATTR(sec) || PIC32_IS_BSS_ATTR_WITH_MEMORY_ATTR(sec) ||
          PIC32_IS_PERSIST_ATTR(sec))
        elf_section_type (sec) = SHT_NOBITS;

     /* MERGE-NOTES: ELF flags are not updated correctly by our replacement hook, causing
        unexpected flags, e.g. write for init_array and fini_array. The following
        maps our BFD flags back to ELF flags (yeah, yeah...) and hands off to the
        standard elf section code, having processsed our magic. */
     int flags = sec->flags;
...
#ifdef TARGET_IS_PIC32MX
  if (pic32_gen_stack_usage != STACK_USAGE_DISABLED)
    pic32_su_start_function (S_GET_NAME (symbolP));
#endif
    }
...
static const struct mips_cpu_info mips_cpu_info_table[] =
{
  ...
  { "pic32mx",         0, ASE_DSP | ASE_DSPR2 | ASE_MCU,
...
pic32_attribute (int is_section)
{
   ...
   /* MERGE-NOTES: this was used in a commented-out part of a macro definition */
   //symbolS *symbolp;
   ...
   int unused ATTRIBUTE_UNUSED;
{
  ...
  if (strlen(name) == 0) {
    ...
    *input_line_pointer = c;
    ...
    /* MERGE-NOTES: fixing const cast warnings. not sure why this local
       copy is needed, unless 'name' changes later on. */
    char * region_name_copy = xmalloc (input_line_pointer - name + 1);
    strcpy (region_name_copy, name);
    region_name = (const char *) region_name_copy;
    *input_line_pointer = c;
  }
  ...
  while (!is_it_end_of_statement ()) {
    ...
    char s[MAX_ATTR_LEN];
    unsigned int i,len;
    /* MERGE-NOTES: this declared a char c shadowing the local in the
       function scope. seems safe to remove the shadowing decl altogether. */

    SKIP_COMMA ();
    SKIP_WHITESPACE ();

    c = get_symbol_name (&name_copy);

    /* make a local copy, lower case */
    name_copy = strncpy(s, name_copy, sizeof(s));
    ...
    *input_line_pointer = c;
    ...
    if (strcmp(name_copy, "size") == 0)
      region_size = arg;
    else if (strcmp(name_copy, "origin") == 0)
      region_origin = arg;
    else {
      as_bad (_("Unrecognized memory attribute \'%s\'"), name_copy);
      demand_empty_rest_of_line ();
      break;
    }
  }
