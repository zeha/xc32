...
exp_fold_tree_1 (etree_type *tree)
{
  ...
  switch (tree->type.node_class)
      ...
      else
	{
      ...
      /* MERGE-TODO: dropping this in as-is. there are changes to the guts of this file
         from the upstream which will require re-examining this code. */
#if defined(TARGET_IS_PIC32C)
        // This is a MCHP extension
        //
        // We want the stack to be allocated late by the best-fit allocator, but this
        // makes the symbol _stack unavailable for use in the linker script.
        // We support a single provide statement of the form
        //
        //     PROVIDE(mysymbol=_stack)
        //
        // We will generate "mysymbol" later, at the same time as we generate _stack.
        // Here we detect such a PROVIDE statement and save aside the string "mysymbol"
        // Only the first such Provide encountered is treated
        // this way.  Subsequent ones are simply ignored.
        // four symbols are supported:  _stack, _splim, _sdtcm_stack and _sdtcm_splim
        
        
        if (tree->type.node_class == etree_provide) {
            
            
            /*  check if this is defining something from _stack  */
            
            if (strcmp((tree->assign.src)->name.name,"_stack") == 0) {
                bfd_boolean stack_is_defined = TRUE;
                h = bfd_link_hash_lookup (link_info.hash,"_stack" ,
                                          FALSE, FALSE, TRUE);
                if((h == NULL) || (h->type == bfd_link_hash_new || h->type == bfd_link_hash_undefined))
                    stack_is_defined = FALSE;
                
                if (!stack_is_defined) {
                    
                    if(altNameforStack == NULL){
                        altNameforStack = xmalloc(strlen(tree->assign.dst) + 1);
                        strcpy(altNameforStack,tree->assign.dst);
                    }
                    break;
                }
                h = NULL;
            ...
            /* and check the same for splim */
            if (strcmp((tree->assign.src)->name.name,"_splim") == 0) {
                bfd_boolean splim_is_defined = TRUE;
                h = bfd_link_hash_lookup (link_info.hash,"_splim" ,
                                          FALSE, FALSE, TRUE);
                if((h == NULL) || (h->type == bfd_link_hash_new || h->type == bfd_link_hash_undefined))
                    splim_is_defined = FALSE;
                
                if (!splim_is_defined) {
                    
                    if(altNameforSplim == NULL){
                        altNameforSplim = xmalloc(strlen(tree->assign.dst) + 1);
                        strcpy(altNameforSplim,tree->assign.dst);
                    }
                    break;
                }
                h = NULL;
            }
            /* and check the same for _sdtcm_stack */
            if (strcmp((tree->assign.src)->name.name,"_sdtcm_stack") == 0) {
                bfd_boolean tcm_stack_is_defined = TRUE;
                h = bfd_link_hash_lookup (link_info.hash,"_sdtcm_stack" ,
                                          FALSE, FALSE, TRUE);
                if((h == NULL) || (h->type == bfd_link_hash_new || h->type == bfd_link_hash_undefined))
                    tcm_stack_is_defined = FALSE;
                
                if (!tcm_stack_is_defined) {
                    
                    if(altNamefor_dtcm_Stack == NULL){
                        altNamefor_dtcm_Stack = xmalloc(strlen(tree->assign.dst) + 1);
                        strcpy(altNamefor_dtcm_Stack,tree->assign.dst);
                    }
                    break;
                }
                h = NULL;	  
            }
            /* and finally _sdtcm_splim */
            if (strcmp((tree->assign.src)->name.name,"_sdtcm_splim") == 0) {
                bfd_boolean tcm_splim_is_defined = TRUE;
                h = bfd_link_hash_lookup (link_info.hash,"_sdtcm_splim" ,
                                          FALSE, FALSE, TRUE);
                if((h == NULL) || (h->type == bfd_link_hash_new || h->type == bfd_link_hash_undefined)) 
                    tcm_splim_is_defined = FALSE;
                
                if (!tcm_splim_is_defined) {
                    
                    if(altNamefor_dtcm_Splim == NULL){
                        altNamefor_dtcm_Splim = xmalloc(strlen(tree->assign.dst) + 1);
                        strcpy(altNamefor_dtcm_Splim,tree->assign.dst);
                    }
                    break;
                }
                h = NULL;	  
