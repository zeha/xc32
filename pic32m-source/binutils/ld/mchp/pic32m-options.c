/* pic32m option processing for the linker.
...
static const char * pic32_shortopts = "-Dp:"; /* note: leading "-" is important */

static struct option pic32_longopts[] =
{
  { "debug", optional_argument, NULL, PIC32M_DEBUG_OPTION },
  { "debug-sio", no_argument, NULL, PIC32M_DEBUG_SMARTIO_OPTION },
...
*/
int
pic32m_parse_args (int argc, char ** argv)
{
  int        longind;
  int        optc;
  ...
  switch (optc)
    ...
    default:
      ...
      optind =  prevoptind;
      return 0;
    case PIC32M_DEBUG_OPTION:
      pic32_debug = TRUE;
      pic32_debug_enable_scope (optarg);
      break;
    case PIC32M_DEBUG_SMARTIO_OPTION:
    ...
    case PAD_FLASH:
        ...
        if (optarg)
        {
            if ((strstr(optarg, "0x") == 0) && (strstr(optarg, "0X") == 0))
                ...
                (void) sscanf(optarg, "%ld", &pad_flash_arg);
        }
        break;
    case APPLICATION_ID:
