...
/* Remove the input file specified by ENTRY.  */
...
{
  bfd_boolean optional_entry = FALSE;
  ...
  if (!entry->flags.search_dirs)
	...
	einfo (_("%P: cannot find %s (%s): %E\n"),
      ...
      else if ((optional_entry = handle_optional_entry (entry)) == FALSE)
#else
      else
#endif
        einfo (_ ("%P: cannot find %s: %E\n"), entry->local_sym_name);
      if (optional_entry == FALSE)
        {
	...
	entry->flags.search_dirs = FALSE;
        ...
        {
          if (entry->flags.sysrooted && ld_sysroot
              && IS_ABSOLUTE_PATH (entry->local_sym_name))
            {
              einfo (_ ("%P: cannot find %s inside %s\n"),
                     entry->local_sym_name, ld_sysroot);
#if defined(TARGET_IS_PIC32MX)
              /* MERGE-NOTES: these are usually set after reporting the error,
                 but for OPTIONAL support we need to only set them if
                 non-optional, hence the hoisting. */
              entry->flags.missing_file = TRUE;
              input_flags.missing_file = TRUE;
#endif
            }
