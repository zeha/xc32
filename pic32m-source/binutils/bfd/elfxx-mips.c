  ...
  for (s = dynobj->sections; s != NULL; s = s->next)
      ...
      if (CONST_STRNEQ (name, ".rel"))
	{
	  if (s->size != 0)
		...
		As long as the SEC_EXCLUDE is excluded from the list of flags
		used to create a ".rel.dyn" section (see
		mips_elf_rel_dyn_section), then it is mandatory for the *.ld
		files to have a "/DISCARD/ : { *(.rel.dyn) }" line.

		In the absenced of SEC_EXCLUDE & "/DISCARD/ : { *(.rel.dyn) }"
		the s->output_section will point to NULL (with
    ...
    for(abfd= info->input_bfds; abfd != NULL; abfd = abfd->link.next)
    {
        ...
        if ((s != NULL) && (s->output_section != NULL) && (s->size > 0))
        {
            if ((s->output_section->lma + s->output_section->output_offset) == section_address)
            {
                ...
                if (pic32_debug)
                    printf("LG --- UPDATE dinit size %ld\n", s->size);
                ...
                if (pic32_debug)
                    printf("LG ... HOHOHO - dinit 0x%lx size\n", s->size);
			   ...
			   struct bfd_link_info *const info)
{
    ...
    {
        if (pic32_debug)
            printf("LG --- shared_dinit_addr - 0x%lx\n", h_sym->u.def.value);
    }
...
#ifdef TARGET_IS_PIC32MX
  { bfd_mach_pic32mx, bfd_mach_mipsisa32r2 },
#endif
...
#define REPORT_AS_DATA(s)       (PIC32_SECTION_IN_DATA_MEMORY(s) || PIC32_IS_MEMORY_ATTR(s))
    ...
    if (pic32_memory_usage)
    {
        ...
        *rom_dat++ = 0; *rom_dat++ = 0;
        ...
        if (pic32_debug)
            printf("  last template addr written = %lx size %ld\n",
