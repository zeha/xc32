				    ...
				    bfd_boolean *pneeded)
{
  ...
  *pneeded = FALSE;
  ...
  for (; pp < ppend; pp++)
	...
	{
	  /* duplicated code because there may be no hash table available */
	  if (!bfd_is_com_section (p->section))
	    {
	      bfd_size_type symcount;
	      asymbol **symbols;
	      ...
	      if (!(*info->callbacks->add_archive_element) (
		    info, abfd, bfd_asymbol_name (p), &abfd))
		return FALSE;
	      symcount = _bfd_generic_link_get_symcount (abfd);
	      symbols = _bfd_generic_link_get_symbols (abfd);
	      if (!generic_link_add_symbol_list (abfd, info, symcount, symbols))
		return FALSE;
	      *pneeded = TRUE;
	      return TRUE;
	    }
