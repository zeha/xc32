...
/* XC32-1661: mingw toolchain (gcc 7.3-win32) does not like the weak function 
   pointer definition idiom. this is a workaround. */
#ifdef __MINGW32__
...
static bfd_boolean pic32_symbol_checks(struct bfd_link_hash_entry * hash_entry, bfd * abfd)
{
    const char *linked_prefix   = "__linked_";
    const char *exclude_prefix  = "__exclude_";
    const char *sym_name        = hash_entry->root.string;

    if (strstr(sym_name, linked_prefix))
    {
        const char *sec_name = &sym_name[strlen(linked_prefix)];
        asection *s;
    ...
    {
        const char *sec_name = &sym_name[strlen(exclude_prefix)];
        asection *s;
        for (s = abfd->sections; s != NULL; s = s->next)
            if (strcmp(sec_name, s->name) == 0)
                s->flags |= SEC_EXCLUDE;
                        ...
                        struct elf_link_hash_entry * const co_h)
{
    ...
    /* MERGE-NOTES: can sec be set on NULL*/
    elf_merge_st_other (abfd, co_h, isym, NULL, definition, FALSE /* (dynamic = FALSE)*/);
}
#endif
...
error_free_dyn:
  ...
  for (isym = isymbuf, isymend = isymbuf + extsymcount;
       ...
       isym++, sym_hash++, ever = (ever != NULL ? ever + 1 : NULL))
    {
      ...
      const char *name;
      struct elf_link_hash_entry *h;
#if defined(TARGET_IS_PIC32MX) /*__CORESIDENT*/
 ...
 /* lghica co-resident */
	    ...
	    co_h = (struct elf_link_hash_entry *)
	      bfd_link_hash_lookup (info->hash, tmp_name, FALSE, FALSE, FALSE);
	...
	goto error_free_vers;
        ...
        pic32_symbol_checks((struct bfd_link_hash_entry *) *sym_hash, abfd);
#endif
    ...
    if (definition == FALSE)
      {
        ...
        mchp_undefsym_lookup(undefsyms, name, 1, 0);
      }
#endif
    }
