...
#define PRETTY \
	...
	if (d.v.i & HAS_DSP) o += fprintf(stderr, "DSP ");
        ...
        PRETTY
        if (d.v.i & HAS_GIE) o += fprintf(stderr, "GIE ");
        PRETTY
        if (d.v.i & HAS_DUALPARTITION) o += fprintf(stderr, "DP ");
        PRETTY
        if (d.v.i & HAS_DATAFLASH) o += fprintf(stderr, "DF ");
        PRETTY
        if (d.v.i & HAS_ALTREGS) o += fprintf(stderr, "ALT ");
        PRETTY
        if (d.v.i & HAS_DUALCORE) o += fprintf(stderr, "2X ");
        PRETTY
        if (d.v.i & HAS_ISAV4) o += fprintf(stderr, "V4 ");
        fprintf(stderr,"\n");
