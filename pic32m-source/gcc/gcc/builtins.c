...
/* MERGE-NOTES: the smart-io handling here will eventually be shared.
   all PIC32C/PIC32MX guards are temporary. */

#if defined(_BUILD_MCHP_) && defined(TARGET_MCHP_PIC32C)
/* MERGE-NOTES: maybe unneccesary, was for smart-io debug.. */
#include "dumpfile.h"
#include "tree-dump.h"
...
#if defined(_BUILD_MCHP_) && defined(TARGET_MCHP_PIC32C)
static tree rewrite_call_expr_args (location_t, int, tree*, int, tree, int, ...);
#endif
...
#if defined(_BUILD_MCHP_) && defined(TARGET_MCHP_PIC32C)
/* used in smart-io handling */
static tree
rewrite_call_expr_args (location_t loc, int nargs, tree *args, int skip, 
                        tree fndecl, int n, ...)
{
  va_list ap;
  tree t;
  ...
  va_start (ap, n);
  t = rewrite_call_expr_valist (loc, nargs, args, skip, fndecl, n, ap);
  va_end (ap);
...
{
    gcc_assert (format_pos >= 0 && format_pos < nargs);
    
    enum built_in_function fcode = DECL_FUNCTION_CODE (fndecl);
    tree fndecl_s = mchp_get_smartio_fndecl (fcode);
    tree fmt = args[format_pos];
    tree format;
    tree spec;
    tree orig_type = NULL_TREE;
    int n;
    
    /* Strip nop but remember the format argument type. */
    if (TREE_CODE (fmt) == NOP_EXPR)
    {
        orig_type = TREE_TYPE (fmt);
        fmt = TREE_OPERAND (fmt, 0);
    }
    if (!SMARTIO_FORMAT_P (fmt))
        return NULL_TREE;
    
    gcc_assert (SMARTIO_SPEC_P (TREE_CHAIN (fmt)));
    
    format = TREE_VALUE (fmt);
    spec = TREE_VALUE (TREE_CHAIN (fmt));
    if (orig_type)
    {
        format = build1 (NOP_EXPR, orig_type, format);
        spec = build1 (NOP_EXPR, orig_type, spec);
    }
    
    /* If smartio is disabled, just replace the spec/fmt list with the format. */
    if (!TARGET_MCHP_SMARTIO_ENABLED)
    {
        switch (format_pos)
        {
            case 0:
                return rewrite_call_expr_args (loc, nargs, args, format_pos + 1, fndecl, 1,
                                               format);
            case 1:
                return rewrite_call_expr_args (loc, nargs, args, format_pos + 1, fndecl, 2,
                                               args[0], format);
            case 2:
                return rewrite_call_expr_args (loc, nargs, args, format_pos + 1, fndecl, 3,
    ...
    /* Only proceed if the target backend provided a valid smart-io builtin. */
    if (fndecl_s)
    {
        switch (format_pos)
        {
            case 0:
                return rewrite_call_expr_args (loc, nargs, args, format_pos + 1, fndecl_s, 2,
                                               spec, format);
            case 1:
                return rewrite_call_expr_args (loc, nargs, args, format_pos + 1, fndecl_s, 3,
                                               spec, args[0], format);
            case 2:
                return rewrite_call_expr_args (loc, nargs, args, format_pos + 1, fndecl_s, 4,
                                               spec, args[0], args[1], format);
            default:
                gcc_unreachable();
        }
    }
