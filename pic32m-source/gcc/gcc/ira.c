...
equiv_init_movable_p (rtx x, int regno)
{
  ...
  switch (code)
    ...
    case UNSPEC_VOLATILE:
      ...
      /* MERGE-TODO: just _BUILD_MCHP_? */
      /* when code coverage is enabled on PIC32C with -fPIC, we need the PIC
       * register even in the functions that otherwise wouldn't need it but,
       * since the codecov instrumentation code is added very late, in such
       * functions the (SET r (UNSPEC ...)) that initializes the PIC register
       * would be removed by update_equiv_regs () here
       * => we'll prevent that by marking the corresp. UNSPEC source as
       * immovable */
    case UNSPEC:
