...
#ifdef _BUILD_MCHP_
  /* MERGE-NOTES: this used to be a condition, not an assert, but should be the same effect */
  /* MERGE-TODO: however, this means the caller is responsible for satisfying this assert. 
     it seems there is a predicate for whether a node should be output in the
     symbol table, which will need a corresponding change for builtins. */
  gcc_checking_assert (TREE_PUBLIC (t)
                       && (!is_builtin_fn (t) || !cgraph_node::get (t)->definition) /* allow built-ins with definitions */
                       && !DECL_ABSTRACT_P (t)
