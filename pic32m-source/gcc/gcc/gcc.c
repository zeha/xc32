...
#ifdef _BUILD_MCHP_
/* MERGE-NOTES: _BUILD_XC32_ replaced with _BUILD_MCHP_ throughout */
...
static char *convert_white_space_free_mem (char *,int);
#endif

/* The Specs Language
...
/* XC32E-696 - target hook for self specs processed after any user self specs. */
#ifndef FINAL_SELF_SPECS
#define FINAL_SELF_SPECS ""
#endif
static const char *const final_self_specs[] = { FINAL_SELF_SPECS };
#endif
...
{
  char *string;
  const char *prev_string = argbuf.pop ();
  ...
  if (target_system_root && !target_system_root_changed && gcc_exec_prefix)
    {
      char *tmp_prefix = get_relative_prefix (decoded_options[0].arg,
					      standard_bindir_prefix,
					      target_system_root);
#if defined(_BUILD_MCHP_) && defined(__MINGW32__)
  ...
  if (argbuf.length () > 0)
      ...
      for (j = 1; j < decoded_options_count; j++)
	{
	  switch (decoded_options[j].opt_index)
		...
		{
/* MERGE-NOTES: mips implementation looks a little bit different */
#ifdef _BUILD_MCHP_
...
do_spec_1 (const char *spec, int inswitch, const char *soft_matched_part)
{
  ...
  while ((c = *p++))
	...
	switch (c = *p++)
	  ...
	  case 'E':
            /* See comment for case 'S', startfile_spec */
#if defined(_BUILD_MCHP_) && defined(TARGET_MCHP_PIC32MX)
	  ...
	  case 'S':
#if defined(_BUILD_MCHP_) && defined(TARGET_MCHP_PIC32MX) 
            /* FIXME (XC32E-677): MIPS goes to some length to avoid init/fini support
               and calling _init when not C++, which is why there is special
               handling here. */
	    if (strstr (progname,"-gcc") != NULL)
...
#if defined(TARGET_IS_PIC32C) || defined(TARGET_IS_PIC32MX)
/*
  This fix was needed because the do_spec_1 - that is called right after
  eval_spec_function - treats white space (' ' and '\t') as the end of an
  argument. As result the
   "-mdfp=C:/Program Files/Microchip/<path>" will be broke into two arguments.

 */
  if (funcval && IS_ABSOLUTE_PATH (funcval) && ! access (funcval, R_OK))
    funcval = convert_white_space_free_mem((char*)funcval, 0);
#endif
...
set_input (const char *filename)
{
  ...
  /* NOTE: Although we could somehow detect the assembler and C startup files
   * here and restrict this alternate behavior only to them, not doing that
   * should also be ok (this is equiv. to -save-temps but using the output dir
  ...
  if (save_temps_length)
    {
      ...
      /* The length of the directory part (including the separator) or 0, if
       * missing */
	...
	--prefix_length;
      ...
      memcpy (save_temps_prefix + prefix_length, input_basename,
	      basename_length);
      save_temps_prefix[save_temps_length] = '\0';
    }
#endif /* _BUILD_MCHP_ */
...
driver::set_up_specs () const
{
  ...
  for (i = 0; i < ARRAY_SIZE (final_self_specs); i++)
    do_self_spec (final_self_specs[i]);
#endif
	  ...
	  if (input_file_compiler->spec[0] == '#')
	    {
                  ...
                  if (input_file_compiler->spec[1] != 'c')
                     error ("%s: %s compiler not supported on this system",
                            LOCATION_FILE (input_location), &input_file_compiler->spec[1]);
                  else
#endif
...
#if (defined(TARGET_MCHP_PIC32MX) || defined(TARGET_MCHP_PIC32C))
#if defined(LINK_COMMAND_SPEC_SUPPRESS_DEFAULT_SCRIPT)
...
{
  return convert_white_space_free_mem(orig, 1);
}

static char *
convert_white_space_free_mem (char *orig, int free_orig)

#else
static char *
convert_white_space_free_mem (char *orig)
#endif
{
