...
/* PIC32 CP0 access functions */
  ...
  PIC32_BUILTIN_JUMP_TO_SYMBOL,
  PIC32_BUILTIN_JUMP_TO_ADDR_IN_REG
...
static const struct attribute_spec mips_attribute_table[] = {
  ...
  { "longcall",    0, 0, false, true,  true,  false, NULL, NULL },
...
#if defined(TARGET_MCHP_PIC32MX)
      func_name = targetm.strip_name_encoding (mchp_strip_name_encoding (XSTR (func, 0)));
#else
...
#if defined(TARGET_MCHP_PIC32MX)
  fnname = targetm.strip_name_encoding (mchp_strip_name_encoding (XSTR (symbol, 0)));
  alias_name = targetm.strip_name_encoding (mchp_strip_name_encoding (XSTR (alias, 0)));
#else
...
#if defined(TARGET_MCHP_PIC32MX)
  fnname = targetm.strip_name_encoding (mchp_strip_name_encoding (XSTR (fn, 0)));
#else
...
// MERGE_TODO: to be checked if SHADOW_SET_YES is the right enum val to be used
#if defined(TARGET_MCHP_PIC32MX)
  if (cfun->machine->interrupt_handler_p
      && GP_REG_P (regno) 
...
*/
  ...
  stripped_fn_name = targetm.strip_name_encoding (mchp_strip_name_encoding (fnname)) ;
  ...
  MIPS_SUBTARGET_OUTPUT_FUNCTION_PROLOGUE(file);
...
static void
mips_output_function_epilogue (FILE *file)
{
  const char *fnname;
  ...
  MIPS_SUBTARGET_OUTPUT_FUNCTION_EPILOGUE(file);
...
#ifdef TARGET_MCHP_PIC32MX
      /* We should set current function stack usage to 0 in case the prologue
       * is not generated. If we leave it to -1, compiler will complain that
       * stack usage is not supported and we cannot generate the stack usage
       * section.
       */
      if (mchp_stack_usage)
...
#if defined(TARGET_MCHP_PIC32MX)
  if (cfun->machine->vector_dispatch)
    return false;
#endif
...
/* MERGE-NOTES: this file, and probably this library, are gone in gcc-8.
   getting rid of them for now using the macro from mips gofast.h. */
...
#define MIPS_SET_FCSR 1
  ...
  PIC32_BUILTIN (jump_to_symbol, PIC32_BUILTIN_JUMP_TO_SYMBOL, MIPS_VOID_FTYPE),
  PIC32_BUILTIN (jump_to_addr_in_reg, PIC32_BUILTIN_JUMP_TO_ADDR_IN_REG, MIPS_VOID_FTYPE)
...
#if defined(TARGET_MCHP_PIC32MX)
int
mips_builtin_decl_index (int idx)
{
  return mips_get_builtin_decl_index[idx];
}
#endif
...
#define MIPS_ATYPE_V2DI mips_builtin_vector_type (long_long_integer_type_node, V2DImode)
#define MIPS_ATYPE_V2HI mips_builtin_vector_type (intHI_type_node, V2HImode)
...
/* stuff for writing pic32m_builtins.h 
   this file contains prototypes for builtin functions to satisfy 
   MISRA checkers   

       Are you adding new builtins to the mips version of XC32 ???
       if so you must re-create the file pic32m-builtins.h that is part of the build.
       This file is generated by compiling any file (hello.c) with the option

       -m-print-protos.

       
       1) make your changes to add your builtins
       2) Generate the file as above.  Make sure your new builtins are there!
       3) add the disclaimer with updated date to the top of the file
       4) check it into pic32m-gcc
...
// initialize the file
void mchp_init_builtins_h(){


    /* the file might already be present with just the header and disclainer stuff there
       in this case just append to what is there   */
    if (access("pic32m_builtins.h", F_OK) != -1)
        builtins_h = fopen("pic32m_builtins.h","a");
    else
        builtins_h = fopen("pic32m_builtins.h","w");
...
// give the typespecs a declaration and a name
// the typespecs that do not actually occur in mchp supported builtins have
// been left in caps.

const char * INT = "int32_t";
const char * POINTER = "void *";
const char * VOID = "void";
const char * CVPOINTER = "const volatile void *";
const char * CONSTSTRING = "const int8_t * ";
const char * DF = "DF";
const char * SF = "SF";
const char * DI = "int64_t";
const char * SI = "int32_t";
const char * HI = "int16_t";
const char * QI = "int8_t";
const char * UDI = "uint64_t";
const char * USI = "uint32_t";
const char * UHI = "uint16_t";
const char * UQI = "uint8_t";
const char * V2QI = "int8_t __attribute__ ((vector_size(2)))";
const char * V2HI = "int16_t __attribute__ ((vector_size(4)))";
const char * V2SI = "int32_t __attribute__ ((vector_size(8)))";
const char * V2DI = "int64_t __attribute__ ((vector_size(16)))";
const char * V4QI = "int8_t __attribute__ ((vector_size(4)))";
const char * V4HI = "int16_t __attribute__ ((vector_size(8)))";
const char * V4SI = "int32_t __attribute__ ((vector_size(16)))";
const char * V4DI = "int64_t __attribute__ ((vector_size(32)))";
const char * V8QI = "int8_t __attribute__ ((vector_size(8)))";
const char * V8HI = "int16_t __attribute__ ((vector_size(16)))";
const char * V8SI = "int32_t __attribute__ ((vector_size(32)))";
const char * V8DI = "int64_t __attribute__ ((vector_size(64)))";
const char * V16QI = "int8_t __attribute__ ((vector_size(16))";
const char * UV2QI = "uint8_t __attribute__ ((vector_size(2)))";
const char * UV2HI = "uint16_t __attribute__ ((vector_size(4)))";
const char * UV2SI = "uint32_t __attribute__ ((vector_size(8)))";
const char * UV2DI = "uint64_t __attribute__ ((vector_size(16)))";
const char * UV4QI = "uint8_t __attribute__ ((vector_size(4)))";
const char * UV4HI = "uint16_t __attribute__ ((vector_size(8)))";
const char * UV4SI = "uint32_t __attribute__ ((vector_size(16)))";
const char * UV4DI = "uint64_t __attribute__ ((vector_size(32)))";
const char * UV8QI = "uint8_t __attribute__ ((vector_size(8)))";
const char * UV8HI = "uint16_t __attribute__ ((vector_size(16)))";
const char * UV8SI = "uint32_t __attribute__ ((vector_size(32)))";
const char * UV8DI = "uint64_t __attribute__ ((vector_size(64)))";
const char * UV16QI = "uint8_t __attribute__ ((vector_size(16))";
const char * V2SF = "V2SF";
const char * V2DF = "V2DF";
const char * V4SF = "V4SF";
const char * V4DF = "V4DF";

bool mchp_unsupported(const char * t) {

/* this is a hack to pare down the list of builtins printed.
    at present xc32 only supports operations on 4 byte int-type vectors  */
...
void write_builtin_proto(const char * name, uint32_t count,...) {
    va_list args;
    char * nameOfType;
    char parameters[200];
    char returnType[50];
    ...
    nameOfType = va_arg(args,char *);
    if (mchp_unsupported(returnType)) return;
    strcat(returnType,nameOfType);
    for(index = 0; index <count-1 ; index ++) {
	nameOfType = va_arg(args,char *);
        if (mchp_unsupported(nameOfType)) return;
	strcat(parameters,nameOfType);
	if(index < count -2)  strcat(parameters,", ");
    }
    
    va_end(args);
    // do not allow an empty parameter list for MISRA 
    if(parameters[0] == '\0') strcat(parameters,"void");
    fprintf(builtins_h,"%s %s (%s);\n",returnType,name,parameters);
}

// some defs to get the expansion below to be sensible ...

#define WRITE_PROTO_STRING0(A) write_builtin_proto(d->name, 1, A);
#define WRITE_PROTO_STRING1(A, B) write_builtin_proto(d->name, 2, A , B);
#define WRITE_PROTO_STRING2(A, B, C) write_builtin_proto(d->name, 3, A , B, C);
#define WRITE_PROTO_STRING3(A, B, C, D) write_builtin_proto(d->name, 4, A , B, C, D);
#define WRITE_PROTO_STRING4(A, B, C, D, E) write_builtin_proto(d->name, 5, A , B, C, D, E);

// write a single prototype to the file
void mchp_write_builtin_proto(const struct mips_builtin_description *d) {

    /* this is for mchp, so first we eliminate the loongson builtins             */
    /* these all start with the prefix __builtin_loongson_  which has length 19  */
    ...
    switch (d->function_type)
      {
#define DEF_MIPS_FTYPE(NUM, ARGS)					\
  case MIPS_FTYPE_NAME##NUM ARGS:					\
       WRITE_PROTO_STRING##NUM ARGS	          			\
	  break;
#include "config/mips/mips-ftypes.def"
#undef DEF_MIPS_FTYPE
      default:
...
mips_init_builtins (void)
{
  ...
  /* if we are just writing pic32m_builtins.h then initialize the file */
  if (TARGET_WRITE_BUILTINS_H) {
      mchp_init_builtins_h();
  }
#endif
...
#ifdef _BUILD_MCHP_
      gcc_assert (ARRAY_SIZE (mips_builtins)
		  == ARRAY_SIZE (mips_builtin_decls));
...
static rtx
pic32_expand_get_isr_state_builtin (ATTRIBUTE_UNUSED enum insn_code icode, rtx target, ATTRIBUTE_UNUSED enum mips_builtin_type builtintype,
...
static rtx
pic32_expand_set_isr_state_builtin (ATTRIBUTE_UNUSED enum mips_builtin_type builtintype,
                   tree exp)
{
  rtx pat;
  rtx opval;  
...
static rtx
pic32_expand_disable_interrupts_builtin (ATTRIBUTE_UNUSED enum insn_code icode, rtx target, ATTRIBUTE_UNUSED enum mips_builtin_type builtintype,
                   ATTRIBUTE_UNUSED tree exp)
{
  rtx pat =0;
...
static rtx
pic32_expand_enable_interrupts_builtin (ATTRIBUTE_UNUSED enum insn_code icode, rtx target, ATTRIBUTE_UNUSED enum mips_builtin_type builtintype,
                   ATTRIBUTE_UNUSED tree exp)
{
  rtx pat;
		     ...
		     machine_mode mode, int ignore)
{
  ...
  switch (d->builtin_type)
    ...
    case PIC32_BUILTIN_JUMP_TO_SYMBOL:
    case PIC32_BUILTIN_JUMP_TO_ADDR_IN_REG:
      {
        tree arg0 = CALL_EXPR_ARG (exp, 0);

        gcc_assert ((TREE_CODE (arg0) == ADDR_EXPR)
                    && "ADDR_EXPR operand expected");
        tree sub_arg0 = TREE_OPERAND (arg0, 0);

        gcc_assert ((TREE_CODE (sub_arg0) == ARRAY_REF)
                    && "ARRAY_REF operand expected");
        sub_arg0 = TREE_OPERAND (sub_arg0, 0);

        gcc_assert ((TREE_CODE (sub_arg0) == STRING_CST)
                    && "STRING_CST operand expected");
        rtx fun = gen_rtx_SYMBOL_REF (Pmode, TREE_STRING_POINTER (sub_arg0));

        rtx call_insn;
        if (d->builtin_type == PIC32_BUILTIN_JUMP_TO_ADDR_IN_REG)
          call_insn = emit_call_insn (
              gen_pic32_jump_to_addr_in_reg (fun, gen_rtx_REG (SImode, 26)));
        else
          call_insn = emit_call_insn (gen_pic32_jump_to_symbol (fun));
        return emit_barrier_after (call_insn);
      }
...

void
mips_bit_clear_info (ATTRIBUTE_UNUSED enum machine_mode mode, unsigned HOST_WIDE_INT m,
