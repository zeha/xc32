...
#include "sbitmap.h"
#ifdef _BUILD_MCHP_
/* needed for subtarget builtin expand stuff. this in turn needs
   gimple.h, brought in above. */
#include "builtins.h"
...
#ifdef _BUILD_MCHP_
unsigned int g_ARM_BUILTIN_MAX = ARM_BUILTIN_MAX;
#endif
...
arm_init_builtins (void)
{
  ...
  /*
       Are you adding new builtins to the arm version of XC32 ???
       if so you must re-create the file pic32c-builtins.h that is part of the build.
       This file is generated by compiling any file (hello.c) with the option
 
       -m-print-protos.

       It is important to supply a -mprocessor option for a part that supports all builtins.
       At the time of writing -mprocessor=32CZ2051CA90176 is adequate.
       ...
       4) check it into pic32c-gcc

   */
  if (TARGET_PRINT_BUILTINS || TARGET_WRITE_BUILTINS_H) {
    /* print all initialized target builtins. subtarget builtins must
       be printed by the subtarget init routine(s). */
    unsigned int i;
    for (i = 0; i != (unsigned int) ARM_BUILTIN_MAX; ++i)
      mchp_print_builtin_function(arm_builtin_decls[i]);
  }
...
arm_builtin_decl (unsigned code, bool initialize_p ATTRIBUTE_UNUSED)
{
  if (code >= ARM_BUILTIN_MAX)
    {
#if defined(TARGET_MCHP_PIC32C) && defined(PIC32C_SUBTARGET_BUILTIN_DECL)
      return PIC32C_SUBTARGET_BUILTIN_DECL (code);
#else
...
#ifdef _BUILD_MCHP_
#ifdef PIC32C_SUBTARGET_EXPAND_BUILTIN
  if (fcode >= ARM_BUILTIN_MAX)
    return PIC32C_SUBTARGET_EXPAND_BUILTIN (fcode, exp, target);
#endif
#endif
...
#ifdef PIC32C_SUBTARGET_GIMPLE_FOLD_BUILTIN
        if (fcode >= ARM_BUILTIN_MAX)
            return PIC32C_SUBTARGET_GIMPLE_FOLD_BUILTIN (gsi);
#endif
    }
    return changed;
}
