...
/* MERGE-TODO: this file is basically two different things in the mips/arm 
   versions. fix it. */
...
void mchp_print_builtin_function (const_tree t)
{
  if (TARGET_WRITE_BUILTINS_H && t && DECL_P(t))
	pretty_tree_with_prototype(const_cast<tree> (t));
...
void
mchp_handle_config_pragma (ATTRIBUTE_UNUSED struct cpp_reader *pfile)
{
  enum cpp_ttype tok;
  tree tok_value;
  ...
  while (1)
    {
      ...
      tok = pragma_lex (&tok_value);
      if (tok == CPP_NAME)
        mchp_handle_configuration_setting (setting_name,
          (const unsigned char *)IDENTIFIER_POINTER (tok_value));
      else if (tok == CPP_NUMBER)
        {
          //host_integerp (..., 0) with tree_fits_shwi_p
          if (tree_fits_shwi_p (tok_value))
          {
            #define MAX_VALUE_NAME_LENGTH 22
            unsigned HOST_WIDE_INT i;
            //Replace tree_low_cst (..., 0) with tree_to_shwi
            i = tree_to_shwi (tok_value);
            value_name = (unsigned char*)xcalloc(MAX_VALUE_NAME_LENGTH,1);
            snprintf((char *)value_name, MAX_VALUE_NAME_LENGTH, "%llu", i);
...
void
mchp_handle_configset_pragma (ATTRIBUTE_UNUSED struct cpp_reader *pfile, const char* set)
{
  enum cpp_ttype tok;
  tree tok_value;
  static int shown_no_config_warning = 0;
