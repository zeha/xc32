...
/* This is at least the string length of the concatenation of all
   print_char_table conversion specifiers. */
#define MAX_PRINT_CHAR_LEN (21+1)
...
#if defined(_BUILD_MCHP_) && defined(TARGET_MCHP_PIC32C)
  { "o", 0, STD_C89, { T89_UI,  T99_UC,  T89_US,  T89_UL,  T9L_ULL, TEX_ULL, T99_ST,  T99_UPD, T99_UIM, BADLEN,  BADLEN,  BADLEN }, "-wp0#",     "i",  NULL },
  { "xX", 0, STD_C89, { T89_UI,  T99_UC,  T89_US,  T89_UL,  T9L_ULL, TEX_ULL, T99_ST,  T99_UPD, T99_UIM, BADLEN,  BADLEN,  BADLEN }, "-wp0#",     "i",  NULL },
#else
...
#if defined(_BUILD_MCHP_) && defined(TARGET_MCHP_PIC32C)
  { "o",     1, STD_C89, { T89_UI,  T99_UC,  T89_US,  T89_UL,  T9L_ULL, TEX_ULL, T99_ST,  T99_UPD, T99_UIM, BADLEN,  BADLEN,  BADLEN }, "*w",   "W",   NULL },
  { "xX",    1, STD_C89, { T89_UI,  T99_UC,  T89_US,  T89_UL,  T9L_ULL, TEX_ULL, T99_ST,  T99_UPD, T99_UIM, BADLEN,  BADLEN,  BADLEN }, "*w",   "W",   NULL },
#else
...
static void check_format_info (function_format_info *, tree,
                               vec<location_t> *,
			       bool warn, bool annot_smartio, bool int_only);
static tree check_format_info_main (format_check_results *, 
				    ...
				    vec<location_t> *);
/* callback for check_function_arguments_recurse_update. */
static void check_format_arg_update (void *, tree *, unsigned HOST_WIDE_INT);
#else
static void check_format_info (function_format_info *, tree,
                               vec<location_t> *);
...
#if defined(_BUILD_MCHP_) && defined(TARGET_MCHP_PIC32C)
          check_format_info (&info, params, arglocs, warn, annot, int_only);
                   ...
                   )
{
...
#if defined(_BUILD_MCHP_) && defined(TARGET_MCHP_PIC32C)
  /* capture this for use before it's advanced below */
  tree format_param = params;
#endif
  ...
  check_function_arguments_recurse_update (check_format_arg_update, &format_ctx,
					   &format_tree, arg_num);
...
#if defined(_BUILD_MCHP_) && defined(TARGET_MCHP_PIC32C)
static void
check_format_arg (void *ctx, tree format_tree, unsigned HOST_WIDE_INT arg_num) 
{
  tree tmp_format_tree = format_tree;
  check_format_arg_update (ctx, &tmp_format_tree, arg_num);
}
#endif

static void 
#if defined(_BUILD_MCHP_) && defined(TARGET_MCHP_PIC32C)
check_format_arg_update (void *ctx, tree *format_tree_p,
		         unsigned HOST_WIDE_INT arg_num)
#else
...
#if defined(_BUILD_MCHP_) && defined(TARGET_MCHP_PIC32C)
  const format_char_info *find_format_char_info (char format_char, int &index, bool warn = false);
#endif
...
#if defined(_BUILD_MCHP_) && defined(TARGET_MCHP_PIC32C)
argument_parser::find_format_char_info (char format_char, int &fcx, bool warn)
#else
...
#if defined(_BUILD_MCHP_) && defined(TARGET_MCHP_PIC32C)
  fcx = (int) (fci - fki->conversion_specs);
#endif
...
{
  int dummy;
  return find_format_char_info (format_char, dummy, true);
}
#endif
...
/* Do the main part of checking a call to a format function.  FORMAT_CHARS
   ...
   Returns a format tree which is either format_tree, or a new COMPOUND_EXPR
   tree whos first parameter is a list containing the set of all conversion
   specifiers found.
   */
/* MCHP-TODO: this is a bit of a mess as it was merged from gcc 6.x in which
   most of the guts of format checking and warning were exposed here.
   Now, there is the shiny arg_parser to use instead; meaning smart-io checking
   should be refactored and moved out of this warning-focused code. */
#if defined(_BUILD_MCHP_) && defined(TARGET_MCHP_PIC32C)
static tree 
#else
static void 
...
/* local-use macro to deal with return statements. */
#if defined(_BUILD_MCHP_) && defined(TARGET_MCHP_PIC32C)
#define __return__(x) return x
#else
#define __return__(x) return
#endif
  ...
  while (*format_chars != 0)
	...
	= arg_parser.find_format_char_info (format_char, fcx, warn);
#else
      const format_char_info * const fci
...
#endif
	{
	  flag_chars.validate (fki, fci, flag_specs, format_chars,
			       format_string_cst, format_string_loc,
			       orig_format_chars, format_char,
			       quotdirs.length () > 0);
	  ...
	  if (!arg_parser.handle_conversions (fci, len_modifier, wanted_type,
					      wanted_type_name, arg_num, params,
					      format_char))
	    continue;
	  ...
	  if (!arg_parser.check_argument_type (fci, len_modifier, wanted_type,
					       wanted_type_name, suppressed,
					       arg_num, params, alloc_flag,
					       format_start, type_start,
					       fmt_param_loc, format_char))
	    __return__ (format_tree);
	} /* if (warn) */
    } /* end while */
#if defined(_BUILD_MCHP_) && defined(TARGET_MCHP_PIC32C)
  if (warn) 
	...
	format_warning_at_char (format_string_loc, format_string_cst,
      ...
      if (info->first_arg_num != 0 && params != 0 && has_operand_number <= 0)
	{
	  res->number_other--;
	  res->number_extra_args++;
	}
	...
	format_warning_at_char (format_string_loc, format_string_cst,
				color_begin - orig_format_chars, OPT_Wformat_,
				"unterminated color directive");
    }

  /* For smart-io functions, make the format a compound with the 
  ...
  if (!conv_set)
    {
      ...
      //  with fmt_s
      arg_parser.find_format_char_info ('s', fcx, warn);
      gcc_assert (fcx >= 0);
      conv_set |= (1 << fcx);
    }
  if (annot && conv_set)
	...
	if ((s & 0x1) && (conv_specs[i].std == STD_C89 || conv_specs[i].std == STD_C99)
      ...
      if (p != spec_str)
        {
          tree spec_lit = build_string_literal ((p - spec_str) + 1, spec_str);
          return build_smartio_format (format_tree, spec_lit);
        } 
    }
#endif
#endif /* _BUILD_MCHP_ */
