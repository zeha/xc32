...
/* MERGE-TODO: these may be common? */
...
{
/* MERGE-NOTES: ported from xc32/final.c/remap_debug_filename routine */
#if defined(_BUILD_MCHP_) && defined(TARGET_MCHP_PIC32MX)
  /* remap_filename may return the original string or a ggc-allocated
     remap, so it is only safe to call unbackslashify if we ensure
     the string is ggc-allocated. */
  const char *remapped = remap_filename (debug_prefix_maps, filename);
  size_t len = strlen (remapped) + 1;
  char *fixed = (char *) ggc_alloc_atomic (len);
  memcpy (fixed, remapped, len);
  unbackslashify (fixed);
  return fixed;
#else
