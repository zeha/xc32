...
{
  /* MERGE-NOTES did we add DECL_UNIQUE_SECTION checks here? it's still
     defined in tree.h but appears unused now.
     This is required to allow our magic for attributes to work, so I'm 
     adding mchp guards for clarity. */
...
get_variable_section (tree decl, bool prefer_noswitch_p)
{
  ...
  if (DECL_COMMON (decl)
#ifdef _BUILD_MCHP_
      && lookup_attribute ("region", DECL_ATTRIBUTES (decl)) == NULL
#endif
     )
    {
...
#ifdef _BUILD_MCHP_
  if (1)
    return targetm.asm_out.select_section (decl, reloc, get_variable_align (decl));
#endif
...
#if defined(TARGET_MCHP_PIC32C) || defined(TARGET_MCHP_PIC32MX)
  /* XC32-1507: our encodings (using '|') are not stripped by the target hook,
     but need to be stripped before we can find the identifier. */
  real_name = targetm.strip_name_encoding (mchp_strip_name_encoding (name));
#else
      ...
      if (GET_CODE (x) == SYMBOL_REF)
	{
	  if (CONSTANT_POOL_ADDRESS_P (x))
	  ...
	  else if (TREE_CONSTANT_POOL_ADDRESS_P (x))
	    {
	      ...
	      if (!TREE_ASM_WRITTEN (DECL_INITIAL (decl)))
		{
/* MERGE-NOTES: ported from xc32/varasm.c/mark_constant routine */
/* also, MIPS only */
#if defined(_BUILD_MCHP_) && defined(TARGET_IS_PIC32MX)
...
categorize_decl_for_section (const_tree decl, int reloc)
{
  ...
  if (TREE_CODE (decl) == FUNCTION_DECL)
  ...
  else if (VAR_P (decl))
    {
      ...
      /* XC32-1736: this applies to both PIC32MX and PIC32C */
      /* XC32-1742: except when a space() attr is present */
      if (!lookup_attribute ("space", DECL_ATTRIBUTES (decl))
    	  && bss_initializer_p (decl, flag_data_sections))
#else
...
#if defined(_BUILD_MCHP_) && defined(TARGET_MCHP_PIC32MX)
              name = IDENTIFIER_POINTER (DECL_ASSEMBLER_NAME (CONST_CAST_TREE (decl)));
...
switch_to_section (section *new_section)
{
  ...
  switch (SECTION_STYLE (new_section))
    {
    case SECTION_NAMED:
/* MERGE-NOTES: these come from MIPS and were subsequently disabled for ARM.
