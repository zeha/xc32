...
#if __cpp_rtti
  /* we should really exclude EH stuff by source file, but for now
     make sure this compiles for our no-exception build.   
     NB: !__cpp_exceptions is used as a proxy for -fno-rtti. */
  if (forced_unwind)
...
#ifdef __thumb__
#ifdef __XC__
asm ("  .pushsection .text.__cxa_end_cleanup, code\n"
#else
...
"	.size __cxa_end_cleanup, . - __cxa_end_cleanup\n"
"	.popsection\n");
#else
#ifdef __XC__
asm ("  .pushsection .text.__cxa_end_cleanup, code\n"
